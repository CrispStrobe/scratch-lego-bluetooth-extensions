...
// ===== CUSTOM ARGUMENT TYPE FOR 3X3 COLOR MATRIX =====
// Extend ArgumentType to include our custom field
var MATRIX_3X3_COLOR = 'matrix_3x3_color';

// Add the new argument type
ArgumentType.MATRIX_3X3_COLOR = MATRIX_3X3_COLOR;

// ===== FIELD IMPLEMENTATION FOR 3X3 COLOR MATRIX =====
/**
 * Class for a 3x3 color matrix field.
 * Creates a clickable 3x3 grid where each cell can be set to different colors and brightness levels.
 */
var FieldMatrix3x3Color = /*#__PURE__*/function () {
  function FieldMatrix3x3Color(value, validator, opt_config) {
    _classCallCheck(this, FieldMatrix3x3Color);
    this.value_ = value || '000000000000000000000000000'; // Default: all off
    this.validator_ = validator;
    this.config_ = opt_config || {};

    // Color mapping (same as LEGO 3x3 matrix protocol)
    this.colors_ = ['#000000',
    // 0: off (black)
    '#FF00FF',
    // 1: magenta
    ...
    this.selectedColor_ = 9; // Default to red
    this.selectedBrightness_ = 5; // Default brightness
  ...
  _createClass(FieldMatrix3x3Color, [{
    ...
    value: function createDom_() {
      ...
      this.fieldGroup_ = document.createElementNS('http://www.w3.org/2000/svg', 'g');
      this.fieldGroup_.setAttribute('class', 'matrix3x3-field');

      // Background rectangle
      this.background_ = document.createElementNS('http://www.w3.org/2000/svg', 'rect');
      this.background_.setAttribute('x', '0');
      this.background_.setAttribute('y', '0');
      this.background_.setAttribute('width', '90');
      this.background_.setAttribute('height', '90');
      this.background_.setAttribute('fill', '#2C2C2C');
      this.background_.setAttribute('stroke', '#666');
      this.background_.setAttribute('stroke-width', '1');
      this.background_.setAttribute('rx', '4');
      this.fieldGroup_.appendChild(this.background_);

      // Create 3x3 grid of cells
      this.cells_ = [];
      for (var row = 0; row < 3; row++) {
        for (var col = 0; col < 3; col++) {
          var cell = this.createCell_(row, col);
          this.cells_.push(cell);
          this.fieldGroup_.appendChild(cell);
        }
      }

      // Color picker panel (initially hidden)
      this.createColorPicker_();
      this.updateDisplay_();
      return this.fieldGroup_;
    }
  }, {
    key: "createCell_",
    value: function createCell_(row, col) {
      var _this = this;
      ...
      rect.setAttribute('x', col * 30 + 3);
      rect.setAttribute('y', row * 30 + 3);
      ...
      cellGroup.cellRect = rect;
      return cellGroup;
    }
  }, {
    key: "createColorPicker_",
    value: function createColorPicker_() {
      // Color picker container
      this.colorPicker_ = document.createElementNS('http://www.w3.org/2000/svg', 'g');
      this.colorPicker_.setAttribute('class', 'color-picker');
      this.colorPicker_.style.display = 'none';

      // Color picker background
      var pickerBg = document.createElementNS('http://www.w3.org/2000/svg', 'rect');
      pickerBg.setAttribute('x', '95');
      pickerBg.setAttribute('y', '0');
      pickerBg.setAttribute('width', '120');
      pickerBg.setAttribute('height', '90');
      pickerBg.setAttribute('fill', '#1C1C1C');
      pickerBg.setAttribute('stroke', '#666');
      pickerBg.setAttribute('stroke-width', '1');
      pickerBg.setAttribute('rx', '4');
      this.colorPicker_.appendChild(pickerBg);

      // Color swatches (3 rows of 4, with off button)
      var colorLayout = [[0, 9, 8, 7],
      // off, red, orange, yellow
      [6, 5, 4, 3],
      // green, mint, turquoise, blue  
      [2, 1, 10, -1] // violet, magenta, white, [brightness]
      ];
      this.colorButtons_ = [];
      for (var row = 0; row < 3; row++) {
        for (var col = 0; col < 4; col++) {
          ...
          if (colorId === -1) {
            // Brightness control
            this.createBrightnessControl_(97 + col * 28, 2 + row * 28);
          } else {
            var colorBtn = this.createColorButton_(colorId, 97 + col * 28, 2 + row * 28);
            this.colorButtons_.push(colorBtn);
            this.colorPicker_.appendChild(colorBtn);
          }
        }
      }
      this.fieldGroup_.appendChild(this.colorPicker_);
    }
  }, {
    key: "createColorButton_",
    value: function createColorButton_(colorId, x, y) {
      var _this2 = this;
      var colorBtn = document.createElementNS('http://www.w3.org/2000/svg', 'rect');
      colorBtn.setAttribute('x', x);
      colorBtn.setAttribute('y', y);
      colorBtn.setAttribute('width', '26');
      colorBtn.setAttribute('height', '26');
      colorBtn.setAttribute('fill', this.colors_[colorId]);
      colorBtn.setAttribute('stroke', '#666');
      colorBtn.setAttribute('stroke-width', '1');
      colorBtn.setAttribute('rx', '2');
      colorBtn.setAttribute('class', 'color-button');
      colorBtn.setAttribute('data-color', colorId);

      // Special styling for off button
      if (colorId === 0) {
        colorBtn.setAttribute('stroke-dasharray', '3,2');
      }
      colorBtn.addEventListener('click', function (e) {
        _this2.selectedColor_ = colorId;
        _this2.updateColorButtons_();
        e.stopPropagation();
      });
      return colorBtn;
    }
    ...
    value: function createBrightnessControl_(x, y) {
      var _this3 = this;
      // Brightness slider container
      var brightnessGroup = document.createElementNS('http://www.w3.org/2000/svg', 'g');
      ...
      brightnessGroup.appendChild(bg);

      // Brightness indicator
      this.brightnessIndicator_ = document.createElementNS('http://www.w3.org/2000/svg', 'rect');
      this.brightnessIndicator_.setAttribute('x', x + 2);
      this.brightnessIndicator_.setAttribute('y', y + 2 + (22 - this.selectedBrightness_ * 2.2));
      this.brightnessIndicator_.setAttribute('width', '22');
      this.brightnessIndicator_.setAttribute('height', this.selectedBrightness_ * 2.2);
      this.brightnessIndicator_.setAttribute('fill', '#FFD700');
      brightnessGroup.appendChild(this.brightnessIndicator_);

      // Brightness text
      var brightnessText = document.createElementNS('http://www.w3.org/2000/svg', 'text');
      brightnessText.setAttribute('x', x + 13);
      brightnessText.setAttribute('y', y + 16);
      brightnessText.setAttribute('text-anchor', 'middle');
      brightnessText.setAttribute('font-family', 'Arial, sans-serif');
      brightnessText.setAttribute('font-size', '8');
      brightnessText.setAttribute('fill', '#000');
      brightnessText.textContent = this.selectedBrightness_;
      this.brightnessText_ = brightnessText;
      brightnessGroup.appendChild(brightnessText);

      // Click handler for brightness
      bg.addEventListener('click', function (e) {
        var rect = bg.getBoundingClientRect();
        var y = e.clientY - rect.top;
        _this3.selectedBrightness_ = Math.max(1, Math.min(10, Math.round((26 - y) / 2.6)));
        _this3.updateBrightnessControl_();
        e.stopPropagation();
      });
      this.colorPicker_.appendChild(brightnessGroup);
    }
  }, {
    key: "onCellClick_",
    value: function onCellClick_(index) {
      if (this.colorPicker_.style.display === 'none') {
        // Show color picker
        this.colorPicker_.style.display = 'block';
        this.selectedCell_ = index;
        this.updateColorButtons_();
      } else {
        // Apply current color to cell
    ...
    value: function setCellColor_(index, colorId, brightness) {
      ...
      var opacity = colorId === 0 ? 0.2 : brightness / 10;
      cell.cellRect.setAttribute('fill', color);
      cell.cellRect.setAttribute('fill-opacity', opacity);

      // Update internal value
      var newValue = this.value_.split('');
      newValue[index * 3] = colorId.toString(16).charAt(0);
      newValue[index * 3 + 1] = brightness.toString(16).charAt(0);
      newValue[index * 3 + 2] = '0'; // Reserved
    ...
    value: function updateColorButtons_() {
      var _this4 = this;
      this.colorButtons_.forEach(function (btn) {
        var colorId = parseInt(btn.getAttribute('data-color'));
        if (colorId === _this4.selectedColor_) {
    ...
    value: function updateBrightnessControl_() {
      var y = parseFloat(this.brightnessIndicator_.getAttribute('y').split(' ')[0]);
      var baseY = y - parseFloat(this.brightnessIndicator_.getAttribute('height')) + 2;
      this.brightnessIndicator_.setAttribute('y', baseY + (22 - this.selectedBrightness_ * 2.2));
      this.brightnessIndicator_.setAttribute('height', this.selectedBrightness_ * 2.2);
      this.brightnessText_.textContent = this.selectedBrightness_;
    }
  }, {
    key: "updateDisplay_",
    value: function updateDisplay_() {
      // Parse value and update all cells
      for (var i = 0; i < 9; i++) {
        var colorId = parseInt(this.value_.charAt(i * 3), 16) || 0;
        var brightness = parseInt(this.value_.charAt(i * 3 + 1), 16) || 5;
        this.setCellColor_(i, colorId, brightness);
      }
    }
  }, {
    key: "updateValue_",
    value: function updateValue_() {
      // Trigger change event for Scratch to detect value changes
      if (this.sourceBlock_) {
        this.sourceBlock_.workspace.fireChangeListener(new Blockly.Events.BlockChange(this.sourceBlock_, 'field', this.name, this.getValue(), this.value_));
      }
    }
  }, {
    key: "getValue",
    ...
    value: function setValue(newValue) {
      if (newValue !== this.value_) {
        this.value_ = newValue || '000000000000000000000000000';
        this.updateDisplay_();
      }
    }
  }, {
...
var SpikePrime = /*#__PURE__*/function () {
  function SpikePrime(runtime, extensionId) {
    ...
    setInterval(function () {
      _this5._timer.current = (Date.now() - _this5._timer.start) / 1000;
    }, 10);
  }
  ...
  _createClass(SpikePrime, [{
    ...
    // NEW: REPL-specific command that captures output
  }, {
    key: "sendReplCommand",
    ...
    value: function _onConnect() {
      ...
      setTimeout(function () {
        ...
        _this7.sendRaw('import hub\r\nprint("PYTHON_AVAILABLE")\r\n');
        // Re-request initial state data
        _this7.sendCommand('trigger_current_state', {}, false);
      ...
      try {
        if (dataText.startsWith('SENSORS:')) {
          ...
          if (this._sensors.gestures.hasOwnProperty(gesture)) {
            ...
            setTimeout(function () {
              _this8._sensors.gestures[gesture] = false;
            }, 100);
          }
    ...
    value: function _parseEventResponse(response) {
      ...
      if (gestureMap[response.p]) {
        ...
        setTimeout(function () {
          _this9._sensors.gestures[gestureMap[response.p]] = false;
        }, 100);
      }
    }
  }]);
  ...
  _createClass(Scratch3SpikePrimeBlocks, [{
    ...
    value: function _motorRunForDegrees(ports, direction, degrees) {
      ...
      var promises = ports.map(function (port) {
        var setting = _this10._peripheral.motorSettings[port];

        // Try both standard command and Python method
        var standardCommand = _this10._peripheral.sendCommand('scratch.motor_run_for_degrees', {
          port: port,
        ...
        var altCommand = _this10._peripheral.sendPythonCommand("import hub; hub.port.".concat(port, ".motor.run_for_degrees(").concat(Math.floor(degrees), ", ").concat(setting.speed * direction, ")"));
    ...
    value: function _motorRunTimed(ports, direction, seconds) {
      var _this11 = this;
      var promises = ports.map(function (port) {
        var setting = _this11._peripheral.motorSettings[port];
        var standardCommand = _this11._peripheral.sendCommand('scratch.motor_run_timed', {
          port: port,
          speed: setting.speed * direction,
        ...
        var altCommand = _this11._peripheral.sendPythonCommand("import hub; hub.port.".concat(port, ".motor.run_for_time(").concat(Math.floor(seconds * 1000), ", ").concat(setting.speed * direction, ")"));
        return standardCommand.catch(function () {
          return altCommand;
        });
      });
    ...
    value: function motorStart(args) {
      ...
      var promises = ports.map(function (port) {
        ...
        var standardCommand = _this12._peripheral.sendCommand('scratch.motor_start', {
          port: port,
          speed: setting.speed * direction,
          stall: setting.stallDetection
        });
        var altCommand = _this12._peripheral.sendPythonCommand("import hub; hub.port.".concat(port, ".motor.pwm(").concat(Math.round(setting.speed * direction), ")"));
        return standardCommand.catch(function () {
          return altCommand;
        });
      });
    ...
    value: function motorStop(args) {
      ...
      var promises = ports.map(function (port) {
        ...
        var standardCommand = _this13._peripheral.sendCommand('scratch.motor_stop', {
          port: port,
          stop: setting.stopMode
        });
        var altCommand = _this13._peripheral.sendPythonCommand("import hub; hub.port.".concat(port, ".motor.stop()"));
        return standardCommand.catch(function () {
          return altCommand;
        });
      });
    ...
    value: function motorSetSpeed(args) {
      ...
      ports.forEach(function (port) {
        _this14._peripheral.motorSettings[port].speed = speed;
      });
    }
  }, {
