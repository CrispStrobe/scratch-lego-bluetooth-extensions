...
// ===== ANTON'S DISCOVERIES - HIGH CONFIDENCE =====

// Built-in sound files (from Anton's research)
var HubSoundFiles = ['menu_click', 'menu_fastback', 'menu_program_start', 'menu_program_stop', 'menu_shutdown', 'startup'];

// Display patterns as matrices (HIGH CONFIDENCE - will work)
var DisplayPatterns = {
  heart: '0110001111011101110100000',
  smile: '0000001001000000100010001',
  sad: '0000001001000001000101110',
  angry: '1000100100000000100010001',
  surprised: '0000001111011111100000000',
  wink: '0000000001000000100010001',
  arrow_up: '0010001110111110001000100',
  arrow_down: '0010000100001011101110001',
  arrow_left: '0000000100011101110001000',
  arrow_right: '0000010000111000110100000',
  check: '0000000001000100010001110',
  x: '1000101010001000101010001',
  square: '1111110001100011000111111',
  triangle: '0010001010101011111110000',
  diamond: '0010001010101010101000100',
  plus: '0010000100111110010000100',
  minus: '0000000000111110000000000',
  dot: '0000000000001000000000000',
  frame: '1111110001100011000111111',
  spiral: '1111010000100001000011111'
};
...
// LED colors for center button (from Anton's research)
var CenterLEDColors = {
  OFF: 0,
  PINK: 1,
  PURPLE: 2,
...
// Sound waveforms (from Anton's research)
var SoundWaveforms = {
  SOUND_SIN: 'hub.sound.SOUND_SIN',
  SOUND_SQUARE: 'hub.sound.SOUND_SQUARE',
  SOUND_TRIANGLE: 'hub.sound.SOUND_TRIANGLE',
  SOUND_SAWTOOTH: 'hub.sound.SOUND_SAWTOOTH'
};
...
var SpikePrime = /*#__PURE__*/function () {
  function SpikePrime(runtime, extensionId) {
    ...
    // Enhanced sensor data structure based on Anton's research
    this._sensors = {
      buttons: [0, 0, 0, 0],
      angle: {
        pitch: 0,
    ...
    // Track if Python execution is available
  ...
  _createClass(SpikePrime, [{
    ...
    value: function stopSound() {
      // HIGH CONFIDENCE: Both approaches should work
      this.sendCommand('scratch.sound_stop', {}, false);
      this.sendPythonCommand('hub.sound.stop()');
    }
  }, {
    key: "stopAllMotors",
    value: function stopAllMotors() {
      // HIGH CONFIDENCE: Both approaches should work
      this.sendCommand('scratch.motor_stop_all', {}, false);
    ...
    // HIGH CONFIDENCE: Python command execution via JSON-RPC tunnel
  }, {
    key: "sendPythonCommand",
    value: function sendPythonCommand(pythonCode) {
      var _this2 = this;
      // Try multiple possible tunnel methods
      return this.sendCommand('scratch.python_eval', {
        code: pythonCode
      }, false).catch(function () {
        return _this2.sendCommand('python_eval', {
          code: pythonCode
        }, false);
      }).catch(function () {
        return _this2.sendCommand('eval', {
          code: pythonCode
        }, false);
      }).catch(function () {
        // If direct Python execution isn't available, try to be creative
        console.warn('Python execution not available via JSON-RPC');
        return Promise.resolve();
      });
    }

    // HIGH CONFIDENCE: Anton's hub methods via Python
  }, {
    key: "sendHubCommand",
    value: function sendHubCommand(hubMethod) {
      var pythonCode = "import hub; ".concat(hubMethod);
    ...
    value: function _onConnect() {
      ...
      // Test if Python execution is available
      this.sendPythonCommand('import hub; print("Python available")').then(function () {
        _this3._pythonAvailable = true;
        console.log('Python execution available on hub');

        // Initialize enhanced sensor monitoring using Anton's methods
        _this3._initializeAntonSensors();
      }).catch(function () {
        _this3._pythonAvailable = false;
        console.log('Python execution not available - using basic JSON-RPC only');
      });
    }

    // HIGH CONFIDENCE: Initialize Anton's sensor monitoring methods
  }, {
    key: "_initializeAntonSensors",
    value: function _initializeAntonSensors() {
      if (!this._pythonAvailable) return;

      // Set up periodic sensor updates using Anton's methods
      var sensorScript = "\nimport hub\nimport utime\n\ndef update_sensors():\n    try:\n        # Get gyro rates (Anton's method)\n        yaw_rate, pitch_rate, roll_rate = hub.motion.gyroscope()\n        \n        # Get angles (Anton's method)  \n        yaw_angle, pitch_angle, roll_angle = hub.motion.position()\n        \n        # Get acceleration (Anton's method)\n        accel_x, accel_y, accel_z = hub.motion.accelerometer()\n        \n        # Get temperatures (Anton's method)\n        battery_temp = hub.battery.temperature()\n        hub_temp = hub.temperature()\n        \n        # Print sensor data for parsing\n        print(f\"SENSORS:{yaw_rate},{pitch_rate},{roll_rate},{yaw_angle},{pitch_angle},{roll_angle},{accel_x},{accel_y},{accel_z},{battery_temp},{hub_temp}\")\n        \n    except Exception as e:\n        print(f\"Sensor error: {e}\")\n\n# Run once to initialize\nupdate_sensors()\n";
      ...
      try {
        for (_iterator.s(); !(_step = _iterator.n()).done;) {
          ...
          try {
            ...
            // Check if this is sensor data from Python output
            if (jsonText.startsWith('SENSORS:')) {
              this._parseAntonSensorData(jsonText);
            } else {
    ...
    // HIGH CONFIDENCE: Parse Anton's sensor data format
  }, {
    key: "_parseAntonSensorData",
    value: function _parseAntonSensorData(sensorText) {
      try {
        var data = sensorText.substring(8).split(',').map(parseFloat);
        if (data.length >= 11) {
          this._sensors.gyro = {
            x: data[1],
            y: data[0],
            z: data[2] // Remap to match expected axes
          };
          this._sensors.angle = {
            yaw: data[3],
            pitch: data[4],
          ...
          this._sensors.temperature = data[9];
          this._sensors.hubTemp = data[10];
        }
      } catch (error) {
        console.warn('Error parsing Anton sensor data:', error);
      }
    }
  }, {
    key: "_parseResponse",
    ...
    value: function _parseEventResponse(response) {
      ...
      if (gestureMap[response.p]) {
        ...
        setTimeout(function () {
          _this4._sensors.gestures[gestureMap[response.p]] = false;
        }, 100);
      }
    }
  }]);
  ...
  _createClass(Scratch3SpikePrimeBlocks, [{
    ...
    value: function getInfo() {
      ...
      return {
        ...
        // ===== ANTON'S MOTOR ENHANCEMENTS (HIGH CONFIDENCE) =====
        {
        ...
        // ===== ANTON'S IMU & GYRO (HIGH CONFIDENCE) =====
        {
          opcode: 'getAngle',
          text: formatMessage({
        ...
        // ===== ANTON'S ENHANCED GESTURES (HIGH CONFIDENCE) =====
        {
          opcode: 'whenGesture',
        ...
        // ===== ANTON'S SOUND SYSTEM (HIGH CONFIDENCE) =====
        {
          opcode: 'playHubSound',
        ...
        // ===== ANTON'S STATUS & TEMPERATURE (HIGH CONFIDENCE) =====
        {
        ...
        // ===== ANTON'S BUTTON ENHANCEMENTS (HIGH CONFIDENCE) =====
        {
          ...
          opcode: 'setupButtonCallback',
          text: 'enable [BUTTON] button press timing',
          blockType: BlockType.COMMAND,
          arguments: {
            BUTTON: {
              type: ArgumentType.STRING,
    ...
    value: function _motorRunForDegrees(ports, direction, degrees) {
      ...
      var promises = ports.map(function (port) {
        var setting = _this5._peripheral.motorSettings[port];

        // Try both standard command and Anton's Python method
        var standardCommand = _this5._peripheral.sendCommand('scratch.motor_run_for_degrees', {
          port: port,
        ...
        var antonCommand = _this5._peripheral.sendPythonCommand("import hub; hub.port.".concat(port, ".motor.run_for_degrees(").concat(Math.floor(degrees), ", ").concat(setting.speed * direction, ")"));
    ...
    value: function _motorRunTimed(ports, direction, seconds) {
      ...
      var promises = ports.map(function (port) {
        ...
        var standardCommand = _this6._peripheral.sendCommand('scratch.motor_run_timed', {
          port: port,
          speed: setting.speed * direction,
        ...
        var antonCommand = _this6._peripheral.sendPythonCommand("import hub; hub.port.".concat(port, ".motor.run_for_time(").concat(Math.floor(seconds * 1000), ", ").concat(setting.speed * direction, ")"));
        return standardCommand.catch(function () {
          return antonCommand;
        });
      });
    ...
    value: function motorStart(args) {
      ...
      var promises = ports.map(function (port) {
        ...
        var standardCommand = _this7._peripheral.sendCommand('scratch.motor_start', {
          port: port,
          speed: setting.speed * direction,
          stall: setting.stallDetection
        });
        var antonCommand = _this7._peripheral.sendPythonCommand("import hub; hub.port.".concat(port, ".motor.pwm(").concat(Math.round(setting.speed * direction * 10), ")"));
        return standardCommand.catch(function () {
          return antonCommand;
        });
      });
    ...
    value: function motorStop(args) {
      ...
      var promises = ports.map(function (port) {
        ...
        var standardCommand = _this8._peripheral.sendCommand('scratch.motor_stop', {
          port: port,
          stop: setting.stopMode
        });
        var antonCommand = _this8._peripheral.sendPythonCommand("import hub; hub.port.".concat(port, ".motor.stop()"));
        return standardCommand.catch(function () {
          return antonCommand;
        });
      });
    ...
    value: function motorSetSpeed(args) {
      ...
      ports.forEach(function (port) {
        _this9._peripheral.motorSettings[port].speed = speed;
      });
    }
  }, {
    key: "getPosition",
    ...
    value: function getRelativePosition(args) {
      ...
      // Try Anton's method to get precise relative position
      this._peripheral.sendPythonCommand("import hub; speed, rel_deg, abs_deg, pwm = hub.port.".concat(port, ".motor.get(); print(f\"REL_POS_").concat(port, ":{rel_deg}\")"));
    ...
    value: function getAbsolutePosition(args) {
      ...
      // Try Anton's method to get precise absolute position
      this._peripheral.sendPythonCommand("import hub; speed, rel_deg, abs_deg, pwm = hub.port.".concat(port, ".motor.get(); print(f\"ABS_POS_").concat(port, ":{abs_deg}\")"));
    ...
    value: function getSpeed(args) {
      ...
      // Use Anton's conversion factor for degrees per second
      ...
      return Math.round(speedPercent * 9.3); // Anton's factor for deg/s
    }
  }, {
    ...
    value: function displayText(args) {
      ...
      var antonCommand = this._peripheral.sendPythonCommand("import hub; hub.display.show(\"".concat(text.replace(/"/g, '\\"'), "\")"));
      return standardCommand.catch(function () {
        return antonCommand;
      });
    }
    ...
    value: function displayImage(args) {
      ...
      // Anton's approach  
      var antonImage = symbol.replace(/1/g, '9').replace(/0/g, '_').match(/.{5}/g).join(':');
      var antonCommand = this._peripheral.sendPythonCommand("import hub; hub.display.show(hub.Image(\"".concat(antonImage, "\"))"));
      return standardCommand.catch(function () {
        return antonCommand;
      });
    }
    ...
    value: function displayClear() {
      ...
      var antonCommand = this._peripheral.sendPythonCommand('import hub; hub.display.show(" ")');
      return standardCommand.catch(function () {
        return antonCommand;
      });
    }
    ...
    value: function setPixel(args) {
      ...
      var antonCommand = this._peripheral.sendPythonCommand("import hub; hub.display.pixel(".concat(x, ", ").concat(y, ", ").concat(Math.round(brightness * 9 / 100), ")"));
      return standardCommand.catch(function () {
        return antonCommand;
      });
    }
    ...
    value: function setCenterButtonColor(args) {
      ...
      var antonCommand = this._peripheral.sendPythonCommand("import hub; hub.led(".concat(colorValue, ")"));
      return standardCommand.catch(function () {
        return antonCommand;
      });
    }
    ...
    value: function getGyroRate(args) {
      ...
      // Trigger Anton's gyro reading
      this._peripheral.sendPythonCommand('import hub; yaw_rate, pitch_rate, roll_rate = hub.motion.gyroscope(); print(f"GYRO:{yaw_rate},{pitch_rate},{roll_rate}")');
    ...
    value: function getFilteredGyroRate(args) {
      ...
      // Trigger Anton's filtered gyro reading
      this._peripheral.sendPythonCommand('import hub; yaw_rate, pitch_rate, roll_rate = hub.motion.gyroscope_filter(); print(f"GYRO_FILT:{yaw_rate},{pitch_rate},{roll_rate}")');
      return this._peripheral.gyro[axis] || 0;
    }
  }, {
    key: "getAcceleration",
    ...
    value: function getFilteredAcceleration(args) {
      ...
      // Trigger Anton's filtered accelerometer reading
      this._peripheral.sendPythonCommand('import hub; x, y, z = hub.motion.accelerometer_filter(); print(f"ACCEL_FILT:{x},{y},{z}")');
      return this._peripheral.acceleration[axis] || 0;
    }
  }, {
    key: "resetYaw",
    ...
    key: "wasAntonGesture",
    value: function wasAntonGesture(args) {
      var gesture = Cast.toString(args.GESTURE);

      // Use Anton's gesture detection
      this._peripheral.sendPythonCommand("import hub; result = hub.motion.was_gesture(hub.motion.".concat(gesture, "); print(f\"GESTURE_").concat(gesture, ":{result}\")"));
      return this._peripheral.gestures[gesture.toLowerCase()] || false;
    }
  }, {
    key: "getOrientation",
    value: function getOrientation() {
      // Use Anton's orientation method
      this._peripheral.sendPythonCommand('import hub; orientation = hub.motion.orientation(); print(f"ORIENTATION:{orientation}")');
    ...
    value: function playBeep(args) {
      ...
      var antonCommand = this._peripheral.sendPythonCommand("import hub; hub.sound.beep(".concat(frequency, ", ").concat(duration, ", hub.sound.SOUND_SIN)"));
      return standardCommand.catch(function () {
        return antonCommand;
      });
    }
    ...
    value: function playWaveBeep(args) {
      ...
      var frequency = Cast.toNumber(args.FREQUENCY);
      var duration = Cast.toNumber(args.DURATION);
      var waveformCode = SoundWaveforms["SOUND_".concat(waveform)] || SoundWaveforms.SOUND_SIN;

      // Use Anton's advanced beep method
    ...
    // ===== STATUS & TEMPERATURE (HIGH CONFIDENCE) =====
  }, {
    key: "getBatteryLevel",
    value: function getBatteryLevel() {
      // Trigger Anton's battery reading
      this._peripheral.sendPythonCommand('import hub; level = hub.battery.capacity_left(); print(f"BATTERY:{level}")');
      return this._peripheral.battery || 100;
    }
  }, {
    key: "getBatteryTemperature",
    value: function getBatteryTemperature() {
      // Use Anton's battery temperature method
      this._peripheral.sendPythonCommand('import hub; temp = hub.battery.temperature(); print(f"BATT_TEMP:{temp}")');
      return this._peripheral.temperature || 25;
    }
  }, {
    key: "getHubTemperature",
    value: function getHubTemperature() {
      // Use Anton's hub temperature method
      this._peripheral.sendPythonCommand('import hub; temp = hub.temperature(); print(f"HUB_TEMP:{temp}")');
    ...
    value: function getHubStatus(args) {
      ...
      // Use Anton's supervision method
      this._peripheral.sendPythonCommand("import hub; info = hub.supervision.info(); print(f\"STATUS_".concat(property, ":{info.get('").concat(property, "', 0)}\")"));
      return 0; // Will be updated by the response
    }
    ...
    value: function setDistanceLights(args) {
      ...
      // Use Anton's distance sensor LED control
    ...
    value: function isButtonPressed(args) {
      ...
      // Use Anton's button method
      this._peripheral.sendPythonCommand("import hub; pressed = hub.button.".concat(button, ".is_pressed(); print(f\"BUTTON_").concat(button, ":{pressed}\")"));
      var buttonIndex = {
        left: 0,
        center: 1,
        right: 2
    ...
    key: "setupButtonCallback",
    value: function setupButtonCallback(args) {
      var button = Cast.toString(args.BUTTON);

      // Set up Anton's button callback for press timing
      return this._peripheral.sendPythonCommand("import hub; hub.button.".concat(button, ".callback(lambda time_ms: print(f\"BUTTON_TIME_").concat(button, ":{time_ms}\"))"));
    }
