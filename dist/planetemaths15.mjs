...
var Scratch3PMBlocks = /*#__PURE__*/function () {
  ...
  _createClass(Scratch3PMBlocks, [{
    ...
    value: function setupTranslations() {
      ...
      // ✅ FIX: Pass the ENTIRE translations object to the setup function.
      formatMessage.setup({
        locale: locale,
    ...
    value: function getInfo() {
      return {
        ...
        blocks: [{
          ...
          text: formatMessage({
            id: 'pm.random',
            default: 'pick random from [NUM1] to [NUM2]'
          }),
          blockType: BlockType.REPORTER,
          arguments: {
            NUM1: {
        ...
        menus: {
          PARTIE_ENTIERE: {
            items: this._getMenuItems('c', 12, 'text.c')
          },
          PARTIE_DECIMALE: {
            items: this._getMenuItems('d', 6, 'text.d')
          },
          CHIFFRE_ARRONDIS: {
            items: this._getMenuItems('a', 6, 'text.a')
          },
          LIST_MATHOP: {
            items: ['sqrt', 'sin', 'cos', 'tan', 'asin', 'acos', 'atan', '10 ^']
          },
          LIST_MATHOP2: {
            items: [{
              text: formatMessage({
          ...
          ARRONDIS: {
            items: [{
              text: formatMessage({
                id: 'text.vad',
                default: 'Approximate value by defect'
              }),
              value: 'vad'
            }, {
              text: formatMessage({
                id: 'text.vae',
                default: 'Approximate value by excess'
              }),
              value: 'vae'
            }, {
              text: formatMessage({
    ...
    key: "_getMenuItems",
    value: function _getMenuItems(prefix, count, idPrefix) {
      var items = [];
      for (var i = 1; i <= count; i++) {
        var id = "".concat(idPrefix).concat(i);
        // The default text can be a simple placeholder; formatMessage will find the real one.
        items.push({
          text: formatMessage({
            id: id,
            default: "".concat(prefix).concat(i)
          }),
          value: String(i)
        });
      }
      // Special case for 'c' and 'a' menus starting at 0
      if (prefix === 'c' || prefix === 'a') {
        items.unshift({
          text: formatMessage({
        ...
        if (prefix === 'c') items.shift(); // Remove the duplicated first element for 'c'
      }
      return items;
    }
  }, {
    ...
    value: function arrondis(args) {
      ...
      switch (type) {
        case 'vad':
          return Math.floor(n1 * p) / p;
        case 'vae':
          return Math.ceil(n1 * p) / p;
        case 'arrondi':
          return Math.round(n1 * p) / p;
      }
      return 0;
    }
  }, {
    ...
    value: function multiple(args) {
      ...
      var n2 = Cast.toNumber(args.NUM2);
      if (Number.isInteger(n1) && Number.isInteger(n2)) {
        if (Cast.toString(args.choix1) === 'multiple') {
          return n1 % n2 === 0;
        }
        if (Cast.toString(args.choix1) === 'diviseur') {
          return n1 !== 0 && n2 % n1 === 0;
        }
      }
      return false;
    ...
    value: function extract(args) {
      ...
      var len = Cast.toNumber(args.NUM2) - from;
      if (from < 0 || len < 0 || from >= str.length) {
        return '';
      }
