...
// SPIKE Prime specific constants
...
var SpikePrime = /*#__PURE__*/function () {
  ...
  _createClass(SpikePrime, [{
    ...
    value: function beep(freq, time) {
      var code = "\nimport hub\nfrom runtime import VirtualMachine\n\nhub.sound.beep(freq=".concat(freq, ", time=").concat(time, ")\n        ").trim();
    ...
    value: function scan() {
      ...
      // Use BLE service UUID instead of device class
      this._bt = new BT(this._runtime, this._extensionId, {
        serviceUuid: SPIKE_SERVICE_UUID,
        // Fallback to device class for older implementations
        majorDeviceClass: 8,
        minorDeviceClass: 1
    ...
    key: "executeCode",
    value: function executeCode(code) {
      if (!this.isConnected()) {
      ...
      return this.sendJSON(message, true);
    }
  }, {
    key: "sendJSON",
    value: function sendJSON(json) {
      ...
      var promise = new Promise(function (resolve, reject) {
        ...
        setTimeout(function () {
          if (_this._openRequests[json.i]) {
            delete _this._openRequests[json.i];
    ...
    value: function _onConnect() {
      // Initialize connection with SPIKE Prime
      var initCode = "\nimport hub\nimport motor\nimport motor_pair\nimport color_sensor\nimport distance_sensor\nimport force_sensor\nimport motion_sensor\nfrom hub import button, light_matrix, light, sound\n\n# Send initial status\nprint(\"SPIKE_READY\")\n        ".trim();
      this.executeCode(initCode);
    }
  }, {
    key: "_onMessage",
    value: function _onMessage(params) {
      ...
      try {
        for (_iterator.s(); !(_step = _iterator.n()).done;) {
          ...
          try {
            // Handle both JSON responses and plain text console output
            if (jsonText.trim().startsWith('{') && jsonText.trim().endsWith('}')) {
              var json = JSON.parse(jsonText);
              this._parseResponse(json);
            } else {
              // Handle console output
              this._parseConsoleOutput(jsonText.trim());
            }
          } catch (error) {
            // Not JSON, treat as console output
            this._parseConsoleOutput(jsonText.trim());
          }
        }
      } catch (err) {
    ...
    key: "_parseConsoleOutput",
    value: function _parseConsoleOutput(text) {
      if (text === 'SPIKE_READY') {
        console.log('SPIKE Prime initialized successfully');
        this._startSensorUpdates();
      }
      // Handle other console outputs as needed
    }
  }, {
    key: "_startSensorUpdates",
    value: function _startSensorUpdates() {
      // Start periodic sensor updates
      var sensorCode = "\nimport hub\nimport motor\nimport time\n\ndef update_sensors():\n    while True:\n        try:\n            # Get motor positions\n            motors = {}\n            for port in ['A', 'B', 'C', 'D', 'E', 'F']:\n                try:\n                    motors[port] = {\n                        'position': motor.absolute_position(port),\n                        'speed': motor.velocity(port)\n                    }\n                except:\n                    motors[port] = {'position': 0, 'speed': 0}\n            \n            # Get hub orientation\n            angle = hub.motion_sensor.tilt_angles()\n            \n            # Send sensor data\n            print('SENSOR_DATA:' + str({\n                'motors': motors,\n                'angle': {'pitch': angle[0], 'roll': angle[1], 'yaw': 0}\n            }))\n            \n            time.sleep(0.1)\n        except Exception as e:\n            print('Sensor error: ' + str(e))\n            time.sleep(1)\n\nupdate_sensors()\n        ".trim();
      this.executeCode(sensorCode);
    }
  }, {
    key: "_parseResponse",
    value: function _parseResponse(response) {
      ...
      if (response.m === 'sensor_update') {
        this._updateSensors(response.p);
      }
    }
  }, {
    key: "_updateSensors",
    value: function _updateSensors(sensorData) {
      if (sensorData.motors) {
        for (var port in sensorData.motors) {
          if (SpikePorts.includes(port)) {
            this._portValues[port] = {
              position: (sensorData.motors[port].position + 360) % 360,
              speed: sensorData.motors[port].speed,
              degreesCounted: sensorData.motors[port].position,
              power: Math.abs(sensorData.motors[port].speed)
            };
          }
        }
      }
      if (sensorData.angle) {
        this._sensors.angle = sensorData.angle;
      }
    }
  }]);
  return SpikePrime;
...
var Scratch3SpikePrimeBlocks = /*#__PURE__*/function () {
  ...
  _createClass(Scratch3SpikePrimeBlocks, [{
    ...
    value: function motorRunFor(args) {
      ...
      var code = '';
      var _iterator2 = _createForOfIteratorHelper(ports),
        _step2;
      try {
        for (_iterator2.s(); !(_step2 = _iterator2.n()).done;) {
          ...
          switch (unit) {
            case 'rotations':
              code += "motor.run_for_degrees('".concat(port, "', ").concat(value * 360, ", ").concat(speed, ")\n");
              break;
            case 'degrees':
              code += "motor.run_for_degrees('".concat(port, "', ").concat(value, ", ").concat(speed, ")\n");
              break;
            case 'seconds':
              code += "motor.run_for_time('".concat(port, "', ").concat(value * 1000, ", ").concat(speed, ")\n");
              break;
          }
        }
      } catch (err) {
      ...
      return this._peripheral.executeCode("\nimport motor\n".concat(code, "\n        ").trim());
    }
  }, {
    ...
    value: function motorGoDirectionToPosition(args) {
      ...
      var ports = this._validatePorts(Cast.toString(args.PORT));
      var code = '';
      var _iterator3 = _createForOfIteratorHelper(ports),
        _step3;
      try {
        for (_iterator3.s(); !(_step3 = _iterator3.n()).done;) {
          var port = _step3.value;
          var setting = this._peripheral.motorSettings[port];
          code += "motor.run_to_absolute_position('".concat(port, "', ").concat(position, ", ").concat(setting.speed, ", direction='").concat(direction, "')\n");
        }
      } catch (err) {
        _iterator3.e(err);
      } finally {
    ...
    value: function motorStart(args) {
      ...
      try {
        for (_iterator4.s(); !(_step4 = _iterator4.n()).done;) {
          ...
          var setting = this._peripheral.motorSettings[port];
          var speed = setting.speed * direction;
          code += "motor.run('".concat(port, "', ").concat(speed, ")\n");
        }
      } catch (err) {
        _iterator4.e(err);
      } finally {
    ...
    value: function motorSetSpeed(args) {
      ...
      ports.forEach(function (port) {
        _this2._peripheral.motorSettings[port].speed = speed;
      });
      return Promise.resolve();
    }
  }, {
    ...
    value: function displayImageFor(args) {
      ...
      var image = this._formatImage(symbol, brightness);
      var duration = MathUtil.clamp(Cast.toNumber(args.DURATION) * 1000, 0, 60000);
      return this._peripheral.executeCode("\nimport hub\nimport time\nhub.light_matrix.show_image('".concat(image, "')\ntime.sleep(").concat(duration / 1000, ")\nhub.light_matrix.clear()\n        ").trim());
    }
  }, {
    key: "displayImage",
    ...
    value: function displayText(args) {
      ...
      return this._peripheral.executeCode("\nimport hub\nhub.light_matrix.write('".concat(text, "')\n        ").trim());
    }
  }, {
    key: "displayClear",
    ...
    value: function displaySetPixel(args) {
      ...
      return this._peripheral.executeCode("\nimport hub\nhub.light_matrix.set_pixel(".concat(x - 1, ", ").concat(y - 1, ", ").concat(brightness, ")\n        ").trim());
    }
  }, {
    key: "centerButtonLights",
    value: function centerButtonLights(args) {
      var color = Cast.toNumber(args.COLOR);
      return this._peripheral.executeCode("\nimport hub\nhub.light.color(".concat(color, ")\n        ").trim());
    }
  }, {
    ...
    value: function ultrasonicLightUp(args) {
      ...
      return this._peripheral.executeCode("\nimport distance_sensor\ndistance_sensor.light_up(".concat(lights.join(', '), ", '").concat(port, "')\n        ").trim());
    }
    ...
    value: function beep(args) {
      ...
      time = MathUtil.clamp(time, 0, 3);
      if (time === 0) {
        return Promise.resolve();
      }
      return new Promise(function (resolve) {
        // Convert MIDI note to frequency
        var freq = Math.round(Math.pow(2, (note - 69 + 12) / 12) * 440);
        _this3._peripheral.executeCode("\nimport hub\nhub.sound.beep(freq=".concat(freq, ", time=").concat(time * 1000, ")\n            ").trim());
    ...
    key: "_formatImage",
    value: function _formatImage(symbol, brightness) {
      return symbol.replace(/1/g, brightness).match(/.{5}/g).join(':');
    }
  }, {
    key: "_validatePorts",
