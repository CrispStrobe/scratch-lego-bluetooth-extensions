...
var entry = {
  ...
  description: /*#__PURE__*/React.createElement(FormattedMessage, {
    defaultMessage: "Control games with a PlayStation DualShock 4 controller.",
  ...
  connectingMessage: /*#__PURE__*/React.createElement(FormattedMessage, {
    ...
    id: "gui.extension.dualshock4.connectingMessage"
  }),
  helpLink: 'https://scratch.mit.edu/dualshock4'
};
...
var extensionURL = 'https://cdn.jsdelivr.net/gh/CrispStrobe/scratch-lego-bluetooth-extensions@master/dist/dualshock4.mjs';

// Button mapping based on actual DualShock 4 HID reports
var DS4_BUTTONS = {
  SQUARE: 0,
  X: 1,
  CIRCLE: 2,
...
var DualShock4_HID_Peripheral = /*#__PURE__*/function () {
  function DualShock4_HID_Peripheral(runtime) {
    _classCallCheck(this, DualShock4_HID_Peripheral);
    ...
    this._previousButtons = _toConsumableArray(this.state.buttons);
    console.log('🎮 DualShock 4 WebHID peripheral initialized');
  }
  _createClass(DualShock4_HID_Peripheral, [{
    key: "connect",
    value: function () {
        ...
        return regenerator.wrap(function _callee$(_context) {
          while (1) switch (_context.prev = _context.next) {
            case 0:
              ...
              alert('WebHID is not supported in this browser.\n\nPlease use:\n• Chrome 89+\n• Edge 89+\n• Opera 75+\n\nFirefox and Safari do not support WebHID yet.');
              return _context.abrupt("return");
            case 4:
              _context.prev = 4;
              console.log('🔍 Opening WebHID device picker...');
              console.log('💡 Make sure your DualShock 4 is:');
              console.log('   • Connected via USB cable, OR');
              console.log('   • Paired via Bluetooth and awake (press PS button)');
            ...
            case 11:
              ...
              console.log('❌ No DualShock 4 found with standard IDs');
              console.log('🔍 Trying to show ALL HID devices for debugging...');
              _context.prev = 15;
              _context.next = 18;
              return navigator.hid.requestDevice({
            ...
            case 18:
              ...
              console.log('❌ No HID devices found at all');
              console.log('💡 This suggests:');
              console.log('   • Controller not connected via USB');
              console.log('   • Bluetooth controller not in HID mode');
              console.log('   • Controller using different driver');
              return _context.abrupt("return");
            case 26:
              _context.next = 32;
              break;
            case 28:
              ...
              console.log('❌ Could not show all devices:', _context.t0);
              return _context.abrupt("return");
            case 32:
              if (devices.length) {
                _context.next = 40;
              ...
              console.log('💡 If your controller didn\'t appear in the list:');
              console.log('   • Try connecting via USB cable');
              console.log('   • Make sure it\'s a genuine Sony DualShock 4');
              console.log('   • Press PS button to wake it up');
              console.log('   • Check Windows Device Manager for "HID-compliant game controller"');
              return _context.abrupt("return");
            case 40:
              this.device = devices[0];
              console.log("\uD83C\uDFAE Selected: ".concat(this.device.productName || 'DualShock 4'));
              console.log("\uD83D\uDCCB Device ID: ".concat(this.device.vendorId.toString(16).padStart(4, '0'), ":").concat(this.device.productId.toString(16).padStart(4, '0')));
              _context.next = 45;
              return this.device.open();
            case 45:
              console.log('✅ DualShock 4 Connected via WebHID!');
              console.log('🎉 Controller is ready to use!');
              this.device.addEventListener('inputreport', this._onInputReport.bind(this));

              // Send initial light bar command (blue) to confirm connection
              this._setLightBar(0, 0, 255);
              return _context.abrupt("return", true);
            case 52:
              _context.prev = 52;
              _context.t1 = _context["catch"](4);
              console.error('❌ WebHID connection error:', _context.t1);
              if (_context.t1.name === 'NotAllowedError') {
                console.log('💡 User cancelled or denied permission');
                alert('Connection was cancelled.\n\nTo connect your controller:\n1. Click the connect block again\n2. Select your controller from the list\n3. Click "Connect"');
              } else if (_context.t1.name === 'NotFoundError') {
                console.log('💡 No compatible device found');
                alert('No DualShock 4 controller found.\n\nMake sure your controller is:\n• Connected via USB, or\n• Paired via Bluetooth and awake (press PS button)\n\nThen try the connect block again.');
              } else {
                console.log('💡 Unexpected error:', _context.t1.message);
                alert("Connection failed: ".concat(_context.t1.message, "\n\nTry:\n\u2022 Using a USB cable\n\u2022 Restarting your browser\n\u2022 Checking Windows Device Manager"));
              }
    ...
    value: function disconnect() {
      if (this.device) {
        console.log('🔌 Disconnecting DualShock 4...');
        this.device.close();
        this.device = null;
        this._resetState();
      }
    ...
    value: function _resetState() {
      this.state.buttons.fill(false);
      this.state.sticks.left = {
        x: 0,
        y: 0
      };
      this.state.sticks.right = {
        x: 0,
        y: 0
      };
      this.state.triggers = {
      ...
      this._previousButtons.fill(false);
    }
  }, {
    key: "_onInputReport",
    value: function _onInputReport(event) {
      ...
      // DualShock 4 uses report ID 1 for standard input
      if (reportId !== 1 || data.byteLength < 10) return;
      try {
        // Save previous state for HAT block logic
        this._previousButtons = _toConsumableArray(this.state.buttons);

        // Parse analog sticks (bytes 0-3, range 0-255)
        this.state.sticks.left.x = this._normalizeStick(data.getUint8(0));
        this.state.sticks.left.y = this._normalizeStick(data.getUint8(1));
        this.state.sticks.right.x = this._normalizeStick(data.getUint8(2));
        this.state.sticks.right.y = this._normalizeStick(data.getUint8(3));

        // Parse triggers (bytes 6-7, range 0-255)
        this.state.triggers.l2 = data.getUint8(6) / 255;
        this.state.triggers.r2 = data.getUint8(7) / 255;

        // Parse buttons (bytes 4-5)
        var buttonByte1 = data.getUint8(4); // D-pad + face buttons
        var buttonByte2 = data.getUint8(5); // Shoulder buttons + system buttons

        // D-Pad (lower 4 bits of byte 4)
        var dpad = buttonByte1 & 0x0F;
        this.state.buttons[DS4_BUTTONS.DPAD_UP] = dpad === 0 || dpad === 1 || dpad === 7;
        this.state.buttons[DS4_BUTTONS.DPAD_RIGHT] = dpad === 1 || dpad === 2 || dpad === 3;
        this.state.buttons[DS4_BUTTONS.DPAD_DOWN] = dpad === 3 || dpad === 4 || dpad === 5;
        this.state.buttons[DS4_BUTTONS.DPAD_LEFT] = dpad === 5 || dpad === 6 || dpad === 7;

        // Face buttons (upper 4 bits of byte 4)
        this.state.buttons[DS4_BUTTONS.SQUARE] = (buttonByte1 & 0x10) !== 0;
        this.state.buttons[DS4_BUTTONS.X] = (buttonByte1 & 0x20) !== 0;
        this.state.buttons[DS4_BUTTONS.CIRCLE] = (buttonByte1 & 0x40) !== 0;
        this.state.buttons[DS4_BUTTONS.TRIANGLE] = (buttonByte1 & 0x80) !== 0;

        // Shoulder and system buttons (byte 5)
        this.state.buttons[DS4_BUTTONS.L1] = (buttonByte2 & 0x01) !== 0;
        this.state.buttons[DS4_BUTTONS.R1] = (buttonByte2 & 0x02) !== 0;
        this.state.buttons[DS4_BUTTONS.L2] = (buttonByte2 & 0x04) !== 0;
        this.state.buttons[DS4_BUTTONS.R2] = (buttonByte2 & 0x08) !== 0;
        this.state.buttons[DS4_BUTTONS.SHARE] = (buttonByte2 & 0x10) !== 0;
        this.state.buttons[DS4_BUTTONS.OPTIONS] = (buttonByte2 & 0x20) !== 0;
        this.state.buttons[DS4_BUTTONS.L3] = (buttonByte2 & 0x40) !== 0;
        this.state.buttons[DS4_BUTTONS.R3] = (buttonByte2 & 0x80) !== 0;

        // Additional buttons (byte 6 if available)
        if (data.byteLength > 8) {
          var buttonByte3 = data.getUint8(8);
          this.state.buttons[DS4_BUTTONS.PS] = (buttonByte3 & 0x01) !== 0;
          this.state.buttons[DS4_BUTTONS.TOUCHPAD] = (buttonByte3 & 0x02) !== 0;
        }

        // Check for button press events (for HAT blocks)
        this._checkButtonEvents();

        // Parse battery info if available (byte 10)
        if (data.byteLength > 10) {
          var batteryByte = data.getUint8(10);
          this.state.battery = Math.round((batteryByte & 0x0F) / 15 * 100);
          this.state.charging = (batteryByte & 0x10) !== 0;
        }
      } catch (error) {
        console.error('❌ Error parsing DualShock 4 input report:', error);
      }
    }
  }, {
    key: "_normalizeStick",
    value: function _normalizeStick(rawValue) {
      // Convert 0-255 range to -1 to 1, with deadzone
      var normalized = rawValue / 127.5 - 1;
      return Math.abs(normalized) < 0.08 ? 0 : normalized;
    }
  }, {
    key: "_checkButtonEvents",
    value: function _checkButtonEvents() {
      for (var i = 0; i < this.state.buttons.length; i++) {
        if (this.state.buttons[i] && !this._previousButtons[i]) {
          // Button was just pressed - trigger HAT blocks
          var buttonName = this._getButtonName(i);
          if (buttonName) {
            this._runtime.startHats('dualshock4_whenButtonPressed', {
              BUTTON: buttonName
            });
          }
        }
    ...
    key: "_getButtonName",
    value: function _getButtonName(buttonIndex) {
      for (var _i = 0, _Object$entries = Object.entries(DS4_BUTTONS); _i < _Object$entries.length; _i++) {
        var _Object$entries$_i = _slicedToArray(_Object$entries[_i], 2),
          name = _Object$entries$_i[0],
          index = _Object$entries$_i[1];
        if (index === buttonIndex) return name;
      }
      return null;
    }

    // Output functions (vibration, light bar)
  }, {
    key: "vibrate",
    value: function () {
      var _vibrate = _asyncToGenerator( /*#__PURE__*/regenerator.mark(function _callee2(leftMotor, rightMotor) {
        var _this = this;
        var duration,
          outputReport,
          _args2 = arguments;
        return regenerator.wrap(function _callee2$(_context2) {
          while (1) switch (_context2.prev = _context2.next) {
            ...
            case 3:
              _context2.prev = 3;
              // DualShock 4 output report for vibration and light bar
              outputReport = new Uint8Array(32);
              outputReport[0] = 0x05; // Report ID
              outputReport[1] = 0xFF; // Flags
              outputReport[4] = Math.round(rightMotor * 255); // Right motor (weak)
              outputReport[5] = Math.round(leftMotor * 255); // Left motor (strong)

              // Keep current light bar color (blue by default)
              outputReport[6] = 0; // Red
              outputReport[7] = 0; // Green  
              outputReport[8] = 255; // Blue
              _context2.next = 14;
              return this.device.sendReport(0x05, outputReport);
            case 14:
              console.log("\uD83D\uDCF3 Vibration: Left=".concat(Math.round(leftMotor * 100), "%, Right=").concat(Math.round(rightMotor * 100), "%"));
              if (duration > 0) {
                setTimeout(function () {
                  return _this.stopVibration();
                }, duration * 1000);
              }
              _context2.next = 21;
              break;
            case 18:
              ...
              console.error('❌ Vibration failed:', _context2.t0);
            case 21:
            case "end":
    ...
    key: "stopVibration",
    value: function () {
      var _stopVibration = _asyncToGenerator( /*#__PURE__*/regenerator.mark(function _callee3() {
        return regenerator.wrap(function _callee3$(_context3) {
          while (1) switch (_context3.prev = _context3.next) {
            case 0:
              return _context3.abrupt("return", this.vibrate(0, 0));
            case 1:
            case "end":
      ...
      function stopVibration() {
        return _stopVibration.apply(this, arguments);
      }
    ...
    key: "_setLightBar",
    value: function () {
      var _setLightBar2 = _asyncToGenerator( /*#__PURE__*/regenerator.mark(function _callee4(r, g, b) {
        var outputReport;
        return regenerator.wrap(function _callee4$(_context4) {
          while (1) switch (_context4.prev = _context4.next) {
            ...
            case 2:
              ...
              outputReport[0] = 0x05; // Report ID
              outputReport[1] = 0xFF; // Flags
              outputReport[6] = r; // Red
              outputReport[7] = g; // Green
              outputReport[8] = b; // Blue
              _context4.next = 11;
              return this.device.sendReport(0x05, outputReport);
            case 11:
              console.log("\uD83D\uDCA1 Light bar: RGB(".concat(r, ", ").concat(g, ", ").concat(b, ")"));
              _context4.next = 17;
              break;
            case 14:
              _context4.prev = 14;
              _context4.t0 = _context4["catch"](2);
              console.error('❌ Light bar failed:', _context4.t0);
            case 17:
            case "end":
      ...
      function _setLightBar(_x3, _x4, _x5) {
        return _setLightBar2.apply(this, arguments);
      }
      return _setLightBar;
    }()
  }, {
    key: "setLightBarColor",
    value: function () {
      var _setLightBarColor = _asyncToGenerator( /*#__PURE__*/regenerator.mark(function _callee5(color) {
        var colorNum, r, g, b;
        return regenerator.wrap(function _callee5$(_context5) {
          while (1) switch (_context5.prev = _context5.next) {
            ...
            case 2:
              ...
              colorNum = typeof color === 'string' ? parseInt(color.replace('#', ''), 16) : color;
              r = colorNum >> 16 & 0xFF;
              g = colorNum >> 8 & 0xFF;
              b = colorNum & 0xFF;
              return _context5.abrupt("return", this._setLightBar(r, g, b));
            case 7:
            case "end":
      ...
      function setLightBarColor(_x6) {
        return _setLightBarColor.apply(this, arguments);
      }
      return setLightBarColor;
    }()
  }]);
  return DualShock4_HID_Peripheral;
}();
var Scratch3DualShock4Blocks = /*#__PURE__*/function () {
  function Scratch3DualShock4Blocks(runtime) {
    ...
    this._peripheral = new DualShock4_HID_Peripheral(this.runtime);
  ...
  _createClass(Scratch3DualShock4Blocks, [{
    ...
    value: function getInfo() {
      return {
        id: Scratch3DualShock4Blocks.EXTENSION_ID,
        name: 'DualShock 4 (WebHID)',
        extensionURL: Scratch3DualShock4Blocks.extensionURL,
        blockIconURI: blockIconURI,
        showStatusButton: false,
        // Disable status button - use connect block instead
        blocks: [{
          opcode: 'connect',
          text: '🎮 Connect DualShock 4 (click me!)',
          blockType: BlockType.COMMAND
        }, {
          opcode: 'disconnect',
          text: 'disconnect DualShock 4',
          blockType: BlockType.COMMAND
        }, {
          opcode: 'isConnected',
          text: 'DualShock 4 connected?',
          ...
          opcode: 'getTriggerValue',
          text: '[TRIGGER] trigger value',
          blockType: BlockType.REPORTER,
          arguments: {
            TRIGGER: {
              type: ArgumentType.STRING,
          ...
          opcode: 'vibrate',
          text: 'vibrate [MOTOR] motor at [INTENSITY]% for [DURATION] seconds',
          blockType: BlockType.COMMAND,
          arguments: {
            MOTOR: {
              type: ArgumentType.STRING,
          ...
          opcode: 'setLightBarColor',
          text: 'set light bar color to [COLOR]',
          blockType: BlockType.COMMAND,
          arguments: {
            COLOR: {
          ...
          opcode: 'isCharging',
          text: 'controller charging?',
          blockType: BlockType.BOOLEAN
        }, '---', {
          opcode: 'showWebHIDInfo',
          text: 'show WebHID troubleshooting info',
          blockType: BlockType.COMMAND
        }],
        menus: {
          BUTTONS: {
            acceptReporters: false,
            items: ['X', 'CIRCLE', 'SQUARE', 'TRIANGLE', 'L1', 'R1', 'L2', 'R2', 'SHARE', 'OPTIONS', 'L3', 'R3', 'DPAD_UP', 'DPAD_DOWN', 'DPAD_LEFT', 'DPAD_RIGHT', 'PS', 'TOUCHPAD']
          },
          STICKS: {
            acceptReporters: false,
            items: ['left', 'right']
    ...
    value: function connect() {
      console.log('🎮 Connect block clicked - using WebHID (not Scratch Link)');
    ...
    value: function whenButtonPressed(args) {
      var buttonIndex = DS4_BUTTONS[args.BUTTON];
      if (buttonIndex === undefined) return false;

      // HAT blocks are triggered by the _checkButtonEvents method
      return true;
    }
  }, {
    key: "isButtonPressed",
    value: function isButtonPressed(args) {
      ...
      var buttonIndex = DS4_BUTTONS[args.BUTTON];
      return buttonIndex !== undefined ? this._peripheral.state.buttons[buttonIndex] : false;
    }
  }, {
    key: "getAxisValue",
    value: function getAxisValue(args) {
      if (!this._peripheral.isConnected()) return 0;
      var stick = this._peripheral.state.sticks[args.STICK.toLowerCase()];
      if (!stick) return 0;
      return Math.round(stick[args.AXIS.toLowerCase()] * 100);
    }
  }, {
    key: "getTriggerValue",
    value: function getTriggerValue(args) {
      if (!this._peripheral.isConnected()) return 0;
      var trigger = this._peripheral.state.triggers[args.TRIGGER.toLowerCase()];
      return Math.round(trigger * 100);
    }
  }, {
    key: "vibrate",
    value: function vibrate(args) {
      var motor = Cast.toString(args.MOTOR).toLowerCase();
      var intensity = Cast.toNumber(args.INTENSITY) / 100;
      var duration = Cast.toNumber(args.DURATION);
      var leftMotor = 0;
      var rightMotor = 0;
      if (motor === 'left' || motor === 'both') leftMotor = intensity;
      if (motor === 'right' || motor === 'both') rightMotor = intensity;
      return this._peripheral.vibrate(leftMotor, rightMotor, duration);
    }
  }, {
    key: "stopVibration",
    value: function stopVibration() {
      return this._peripheral.stopVibration();
    }
  }, {
    key: "setLightBarColor",
    value: function setLightBarColor(args) {
      var color = Cast.toString(args.COLOR);
      return this._peripheral.setLightBarColor(color);
    }
  }, {
    key: "getBatteryLevel",
    value: function getBatteryLevel() {
      return this._peripheral.isConnected() ? this._peripheral.state.battery : 0;
    }
  }, {
    key: "isCharging",
    value: function isCharging() {
      return this._peripheral.isConnected() ? this._peripheral.state.charging : false;
    }
  }, {
    key: "showWebHIDInfo",
    value: function showWebHIDInfo() {
      console.log('🎮 DUALSHOCK 4 WEBHID TROUBLESHOOTING');
      console.log('=====================================');
      console.log('');
      console.log('🌐 WebHID Support:');
      console.log("   Available: ".concat(navigator.hid ? '✅ YES' : '❌ NO'));
      console.log("   Browser: ".concat(navigator.userAgent.split(' ').pop()));
      console.log('');
      console.log('🔧 How to Connect:');
      console.log('   1. Click the "🎮 Connect DualShock 4" block (NOT the status button)');
      console.log('   2. Browser will show device picker');
      console.log('   3. Select your controller');
      console.log('   4. Click Connect');
      console.log('');
      console.log('📋 Controller Requirements:');
      console.log('   • Genuine Sony DualShock 4 (v1 or v2)');
      console.log('   • Connected via USB cable, OR');
      console.log('   • Paired via Bluetooth and awake (press PS button)');
      console.log('');
      console.log('🐛 If Controller Not Found:');
      console.log('   • Try USB cable connection');
      console.log('   • Check Windows Device Manager for "HID-compliant game controller"');
      console.log('   • Make sure it\'s not a fake/clone controller');
      console.log('   • Try a different USB port');
      console.log('   • Restart browser');
      console.log('');
      console.log('✅ Supported Browsers:');
      console.log('   • Chrome 89+');
      console.log('   • Edge 89+');
      console.log('   • Opera 75+');
      console.log('');
      console.log('❌ NOT Supported:');
      console.log('   • Firefox (no WebHID support)');
      console.log('   • Safari (no WebHID support)');
      console.log('   • Internet Explorer');
      console.log('');
      console.log('🔍 Current Status:');
      console.log("   Connected: ".concat(this._peripheral.isConnected() ? '✅ YES' : '❌ NO'));
      if (this._peripheral.isConnected()) {
        console.log("   Battery: ".concat(this._peripheral.state.battery, "%"));
        console.log("   Charging: ".concat(this._peripheral.state.charging ? 'Yes' : 'No'));
      }
    }
  }], [{
    key: "EXTENSION_ID",
...
var blockClass = Scratch3DualShock4Blocks;
blockClass = Scratch3DualShock4Blocks;
