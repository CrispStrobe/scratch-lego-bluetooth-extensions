...
var blockIconURI = 'data:image/svg+xml;base64,';
var extensionTranslations = {
  'de': {
    'pm.title': 'Mathe',
    'pm.add': '[NUM1] + [NUM2]',
    'pm.subtract': '[NUM1] - [NUM2]',
    ...
    'pm.multiple': '[NUM1] ist ein [CHOIX] von [NUM2]',
    'pm.arrondis': '[TYPE] von [NUM1] auf [CHIFFRE]',
    'pm.pentiere': '[CHOIX] von [NUM1]',
    'pm.pdecimale': '[CHOIX] von [NUM1]',
    ...
    'menu.ppcm': 'kgV',
    'menu.reste': 'Rest',
    'menu.quotient': 'Quotient',
    'menu.multiple': 'Vielfaches',
    'menu.diviseur': 'Teiler',
    'menu.vad': 'Abrundung',
    'menu.vae': 'Aufrundung',
    'menu.arrondi': 'Rundung',
    'menu.c0': 'Einer',
    'menu.c1': 'Zehner',
    'menu.c2': 'Hunderter',
    'menu.c3': 'Tausender',
  ...
  'fr': {
    ...
    'pm.multiple': '[NUM1] est un [CHOIX] de [NUM2]',
    'pm.arrondis': '[TYPE] de [NUM1] [CHIFFRE]',
    'pm.pentiere': 'chiffre des [CHOIX] de [NUM1]',
    'pm.pdecimale': 'chiffre des [CHOIX] de [NUM1]',
    ...
    'menu.diviseur': 'diviseur',
    'menu.vad': 'valeur approchée par défaut',
    'menu.vae': 'valeur approchée par excès',
    'menu.arrondi': 'arrondi',
    'menu.c0': 'unités',
    'menu.c1': 'dizaines',
    'menu.c2': 'centaines',
    'menu.c3': 'milliers',
...
var Scratch3PMBlocks = /*#__PURE__*/function () {
  ...
  _createClass(Scratch3PMBlocks, [{
    ...
    value: function getInfo() {
      return {
        ...
        blocks: [{
          ...
          text: formatMessage({
            id: 'pm.multiple',
            default: '[NUM1] is a [CHOIX] of [NUM2]'
          }),
          arguments: {
            NUM1: {
              type: ArgumentType.NUMBER,
        ...
        // ✅ FIX: The single '%' must be escaped with another '%'
        {
          opcode: 'pourcent',
          blockType: BlockType.REPORTER,
          text: formatMessage({
          ...
          text: formatMessage({
            id: 'pm.random',
            default: 'pick random from [FROM] to [TO]'
          }),
          arguments: {
            FROM: {
              type: ArgumentType.NUMBER,
          ...
          text: formatMessage({
            id: 'pm.extract',
            default: 'extract characters from [FROM] to [TO] of [STRING]'
          }),
          arguments: {
            FROM: {
              type: ArgumentType.NUMBER,
        ...
        menus: {
          LIST_MATHOP: {
            items: this._generateMenuItems(['sqrt', 'sin', 'cos', 'tan', 'asin', 'acos', 'atan', '10 ^'], 'menu.')
          },
          LIST_MATHOP2: {
            items: this._generateMenuItems(['pgcd', 'ppcm'], 'menu.')
          },
          LIST_MATHOPDIV: {
            items: this._generateMenuItems(['reste', 'quotient'], 'menu.')
          },
          MULTIPLE_DIVISEUR: {
            items: this._generateMenuItems(['multiple', 'diviseur'], 'menu.')
          },
          ARRONDIS: {
            items: this._generateMenuItems(['vad', 'vae', 'arrondi'], 'menu.')
          },
          PARTIE_ENTIERE: {
            items: this._generateMenuItems([{
              text: 'unités',
              value: '0'
            }, {
              text: 'dizaines',
    ...
    // Helper function to build menus from a list
  }, {
    key: "_generateMenuItems",
    value: function _generateMenuItems(options, idPrefix) {
      return options.map(function (opt) {
        var isObject = _typeof$1(opt) === 'object';
        var value = isObject ? opt.value : opt;
        var defaultText = isObject ? opt.text : value;
    ...
    value: function oppose(args) {
      return -1 * Cast.toNumber(args.NUM);
    }
  }, {
    key: "inverse",
    ...
    value: function pourcent(args) {
      return Cast.toNumber(args.NUM) / 100;
    }
  }, {
    key: "mathop",
    ...
    value: function mathop2(args) {
      var operator = Cast.toString(args.OPERATOR).toLowerCase();
      var n1 = Math.round(Cast.toNumber(args.NUM1));
      var n2 = Math.round(Cast.toNumber(args.NUM2));
      if (!Number.isInteger(n1) || !Number.isInteger(n2)) return 0;
      switch (operator) {
        case 'pgcd':
          return this.pgcd(n1, n2);
        case 'ppcm':
          return n1 === 0 || n2 === 0 ? 0 : Math.abs(n1 * n2) / this.pgcd(n1, n2);
        default:
          return 0;
      }
    }
    ...
    value: function mathopdiv(args) {
      ...
      var n1 = Math.round(Cast.toNumber(args.NUM1));
      var n2 = Math.round(Cast.toNumber(args.NUM2));
      if (!Number.isInteger(n1) || !Number.isInteger(n2) || n2 === 0) return 0;
      var result = n1 % n2;
    ...
    value: function arrondis(args) {
      ...
      var n = Cast.toNumber(args.NUM);
      var p = Math.pow(10, Cast.toNumber(args.CHIFFRE));
      switch (type) {
        case 'vad':
    ...
    value: function chiffre_pentiere(args) {
      var num = Math.trunc(Math.abs(Cast.toNumber(args.NUM)));
      var digit = Cast.toNumber(args.CHOIX);
      return Math.trunc(num / Math.pow(10, digit)) % 10;
    }
  }, {
    key: "chiffre_pdecimale",
    value: function chiffre_pdecimale(args) {
      var num = Math.abs(Cast.toNumber(args.NUM));
      var digit = Cast.toNumber(args.CHOIX);
      return Math.trunc(num * Math.pow(10, digit)) % 10;
    }
  }, {
    ...
    value: function multiple(args) {
      ...
      if (Number.isInteger(n1) && Number.isInteger(n2)) {
        if (Cast.toString(args.CHOIX) === 'multiple') {
          return n2 !== 0 && n1 % n2 === 0;
        }
        if (Cast.toString(args.CHOIX) === 'diviseur') {
          return n1 !== 0 && n2 % n1 === 0;
        }
      }
      return false;
    ...
    value: function sommechiffres(args) {
      var value = Math.abs(Cast.toNumber(args.NUM));
    ...
    value: function extract(args) {
      var str = Cast.toString(args.STRING);
      var from = Cast.toNumber(args.FROM) - 1;
      var to = Cast.toNumber(args.TO);
      if (from < 0 || to < from || from >= str.length) return '';
