...
var DualShock4 = /*#__PURE__*/function () {
  function DualShock4(runtime, extensionId) {
    _classCallCheck(this, DualShock4);
    this._runtime = runtime;
    this._extensionId = extensionId;

    // Controller state
    this._buttons = new Array(18).fill(false);
    this._previousButtons = new Array(18).fill(false);
    this._leftStick = {
      x: 0,
      y: 0
    };
    ...
    console.log('🎮 DualShock 4 extension initialized');
  }
  _createClass(DualShock4, [{
    key: "scan",
    value: function scan() {
      console.log('🔍 Starting DualShock 4 scan...');
      if (this._bt) {
        this._bt.disconnect();
      }
      ...
      // Use the exact device class we found: 0x002508
      this._bt = new BT(this._runtime, this._extensionId, {
        majorDeviceClass: 5,
        // Peripheral (from your scan)
        minorDeviceClass: 2 // Minor class 2 (from your scan)
      }, this._onConnect, this._onDisconnect, this._onMessage);
      console.log('✅ Scan initiated for DualShock 4');
    }
  }, {
    key: "connect",
    value: function connect(id) {
      var _this = this;
      console.log("\uD83D\uDD17 Attempting to connect to device: ".concat(id));
      console.log('💡 Make sure the controller is paired in Windows Settings first!');
      console.log('💡 Controller troubleshooting checklist:');
      console.log('   ✅ Controller is paired in Windows (you confirmed this)');
      console.log('   ❓ Controller light bar is solid (not flashing/off)?');
      console.log('   ❓ Controller not connected to PS4/phone/other device?');
      console.log('   ❓ Pressed PS button recently to wake controller?');
      console.log('   ❓ Windows shows "Connected" (not just "Paired")?');
      if (this._bt) {
        ...
        this._connectionTimeout = setTimeout(function () {
          ...
          console.log('   1. Press PS button to wake controller');
          console.log('   2. Check Windows Settings - controller should say "Connected"');
          console.log('   3. Turn off PS4/phone/other devices using the controller');
          console.log('   4. Try removing and re-pairing in Windows Settings');
          console.log('   5. Restart Scratch Link and try again');
          console.log('');
          console.log('💡 Most common cause: Controller connected to another device');
          _this._connected = false;
        ...
        try {
          ...
          console.log('⏳ Waiting for response (timeout in 15 seconds)...');
        } catch (error) {
        ...
        console.error('❌ BT object not initialized');
      }
    }
  }, {
    key: "disconnect",
    ...
    key: "isConnected",
    value: function isConnected() {
      var connected = this._connected;
      console.log("\uD83D\uDCE1 Connection status: ".concat(connected ? 'Connected' : 'Disconnected'));
      return connected;
    }
  }, {
    key: "reset",
    value: function reset() {
      console.log('🔄 Resetting DualShock 4 state...');
    ...
    value: function stopAll() {
      console.log('⏹️ Stopping all DualShock 4 activities...');
      this.stopVibration();
    }
  }, {
    key: "_onConnect",
    value: function _onConnect() {
      var _this2 = this;
      console.log('🎉 DualShock 4 connected successfully via Bluetooth Classic!');
      ...
      setTimeout(function () {
        ...
        _this2._sendTestMessage();
      }, 1000);
    }
  }, {
    key: "_onDisconnect",
    value: function _onDisconnect(error) {
      console.log('🔌 DualShock 4 disconnected');
      if (error) {
    ...
    value: function _onError(error) {
      console.error('❌ DualShock 4 connection error:', error);
      this._connected = false;
      ...
      if (error && typeof error === 'string') {
        if (error.includes('timeout') || error.includes('not found')) {
          ...
          console.log('   1. Pair the controller in Windows Settings first');
          console.log('   2. Make sure controller is in pairing mode (Share + PS buttons)');
          console.log('   3. Check if controller is connected to another device');
          console.log('   4. Try turning controller off and on');
        }
      }
    }
  }, {
    ...
    value: function _onMessage(params) {
      console.log('📨 Received message from DualShock 4:', params);
      if (params && params.message) {
        var data = Base64Util.base64ToUint8Array(params.message);
        console.log('📊 Raw data:', Array.from(data).map(function (b) {
          return b.toString(16).padStart(2, '0');
        }).join(' '));
        this._parseControllerData(data);
      }
    }
  }, {
    key: "_sendTestMessage",
    value: function _sendTestMessage() {
      try {
        // Send a simple test message to see if communication works
        var testData = new Uint8Array([0x01, 0x02, 0x03]); // Simple test pattern

        if (this._bt) {
          this._bt.sendMessage({
            message: Base64Util.uint8ArrayToBase64(testData)
          }).then(function () {
            console.log('✅ Test message sent successfully');
          }).catch(function (error) {
            console.log('⚠️ Test message failed (this might be normal):', error);
          });
        }
      } catch (error) {
        console.log('⚠️ Test message exception (this might be normal):', error);
      }
    }
  }, {
    key: "_parseControllerData",
    value: function _parseControllerData(data) {
      console.log("\uD83D\uDCCA Parsing controller data: ".concat(data.length, " bytes"));
      if (data.length < 12) {
        console.log('⚠️ Data too short, skipping parse');
        return;
      }
      try {
        // Store previous button states
        this._previousButtons = _toConsumableArray(this._buttons);

        // Parse basic button data (simplified)
        if (data.length >= 6) {
          var buttonByte1 = data[5] || 0;
          var buttonByte2 = data[6] || 0;
          console.log("\uD83C\uDFAE Button bytes: ".concat(buttonByte1.toString(16).padStart(2, '0'), " ").concat(buttonByte2.toString(16).padStart(2, '0')));

          // Face buttons
          this._buttons[DS4_BUTTONS.X] = (buttonByte1 & 0x20) !== 0;
          this._buttons[DS4_BUTTONS.CIRCLE] = (buttonByte1 & 0x40) !== 0;
          this._buttons[DS4_BUTTONS.SQUARE] = (buttonByte1 & 0x10) !== 0;
          this._buttons[DS4_BUTTONS.TRIANGLE] = (buttonByte1 & 0x80) !== 0;

          // Log any button presses
          var pressedButtons = [];
          if (this._buttons[DS4_BUTTONS.X]) pressedButtons.push('X');
          if (this._buttons[DS4_BUTTONS.CIRCLE]) pressedButtons.push('Circle');
          if (this._buttons[DS4_BUTTONS.SQUARE]) pressedButtons.push('Square');
          if (this._buttons[DS4_BUTTONS.TRIANGLE]) pressedButtons.push('Triangle');
          if (pressedButtons.length > 0) {
            console.log("\uD83C\uDFAE Buttons pressed: ".concat(pressedButtons.join(', ')));
          }
        ...
        console.error('❌ Error parsing controller data:', error);
      }
    }
  }, {
    ...
    value: function _checkButtonEvents() {
      try {
        for (var i = 0; i < this._buttons.length; i++) {
          if (this._buttons[i] && !this._previousButtons[i]) {
            // Button was just pressed
            var buttonName = this._getButtonName(i);
            console.log("\uD83C\uDFAE Button pressed event: ".concat(buttonName));
        ...
        console.error('❌ Error in button event check:', error);
      }
    }
  }, {
    key: "_getButtonName",
    value: function _getButtonName(buttonIndex) {
      var buttonNames = Object.keys(DS4_BUTTONS);
      for (var _i = 0, _buttonNames = buttonNames; _i < _buttonNames.length; _i++) {
        var name = _buttonNames[_i];
        if (DS4_BUTTONS[name] === buttonIndex) {
    ...
    value: function isButtonPressed(buttonName) {
      var buttonIndex = DS4_BUTTONS[buttonName.toUpperCase()];
      return buttonIndex !== undefined ? this._buttons[buttonIndex] : false;
    }
  }, {
    key: "getStickValue",
    value: function getStickValue(stick, axis) {
      var stickObj = stick === 'left' ? this._leftStick : this._rightStick;
      return stickObj[axis] || 0;
    }
  }, {
    key: "getTriggerValue",
    value: function getTriggerValue(trigger) {
      return trigger === 'l2' ? this._leftTrigger : this._rightTrigger;
    }
  }, {
    key: "vibrate",
    value: function vibrate(motor, intensity, duration) {
      var _this3 = this;
      console.log("\uD83C\uDFAE Vibrate request: ".concat(motor, " motor, ").concat(intensity, "% intensity, ").concat(duration, "s duration"));
      if (!this.isConnected()) {
        console.log('❌ Cannot vibrate: controller not connected');
        return Promise.resolve();
      }
      try {
        intensity = MathUtil.clamp(intensity, 0, 100);
        var leftMotor = motor === 'left' || motor === 'both' ? Math.round(intensity / 100 * 255) : 0;
        var rightMotor = motor === 'right' || motor === 'both' ? Math.round(intensity / 100 * 255) : 0;
        console.log("\uD83D\uDCF3 Vibration values: left=".concat(leftMotor, ", right=").concat(rightMotor));

        // DualShock 4 vibration command
        var vibrateData = new Uint8Array([0x05, 0xFF, 0x00, 0x00, rightMotor, leftMotor, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]);
        console.log("\uD83D\uDCE4 Sending vibration data: ".concat(Array.from(vibrateData).map(function (b) {
          return b.toString(16).padStart(2, '0');
        }).join(' ')));
        if (this._bt) {
          var promise = this._bt.sendMessage({
            message: Base64Util.uint8ArrayToBase64(vibrateData)
          }).then(function () {
            console.log('✅ Vibration command sent successfully');
          }).catch(function (error) {
            console.log('❌ Vibration command failed:', error);
          });
          if (duration > 0) {
            setTimeout(function () {
              console.log('⏰ Stopping vibration after timeout');
              _this3.stopVibration();
            }, duration * 1000);
          }
          return promise;
        }
    ...
    value: function stopVibration() {
      console.log('⏹️ Stopping vibration...');
      if (!this.isConnected()) {
        console.log('❌ Cannot stop vibration: controller not connected');
        return Promise.resolve();
      }
      try {
        var stopData = new Uint8Array([0x05, 0xFF, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]);
        console.log("\uD83D\uDCE4 Sending stop vibration data: ".concat(Array.from(stopData).map(function (b) {
          return b.toString(16).padStart(2, '0');
        }).join(' ')));
        if (this._bt) {
          return this._bt.sendMessage({
            message: Base64Util.uint8ArrayToBase64(stopData)
          }).then(function () {
            console.log('✅ Stop vibration command sent successfully');
          }).catch(function (error) {
            console.log('❌ Stop vibration command failed:', error);
          });
        }
      } catch (error) {
    ...
    value: function setLightBarColor(color) {
      console.log("\uD83D\uDCA1 Setting light bar color to: ".concat(color));
      if (!this.isConnected()) {
        console.log('❌ Cannot set light bar: controller not connected');
        return Promise.resolve();
      }
      try {
        // Convert color to RGB
        var colorNum = typeof color === 'string' ? parseInt(color.replace('#', ''), 16) : color;
        var r = colorNum >> 16 & 0xFF;
        var g = colorNum >> 8 & 0xFF;
        var b = colorNum & 0xFF;
        console.log("\uD83C\uDF08 RGB values: R=".concat(r, ", G=").concat(g, ", B=").concat(b));
        var colorData = new Uint8Array([0x05, 0xFF, 0x00, 0x00, 0x00, 0x00, r, g, b, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]);
        console.log("\uD83D\uDCE4 Sending color data: ".concat(Array.from(colorData).map(function (b) {
          return b.toString(16).padStart(2, '0');
        }).join(' ')));
        if (this._bt) {
          return this._bt.sendMessage({
            message: Base64Util.uint8ArrayToBase64(colorData)
          }).then(function () {
            console.log('✅ Light bar color command sent successfully');
          }).catch(function (error) {
            console.log('❌ Light bar color command failed:', error);
          });
        }
      } catch (error) {
        console.error('❌ Light bar color error:', error);
      }
      return Promise.resolve();
    }
  }]);
...
var extensionURL = 'https://your-server.com/dualshock4.mjs'; // Update this URL
var Scratch3DualShock4Blocks = /*#__PURE__*/function () {
  function Scratch3DualShock4Blocks(runtime) {
    ...
    this._peripheral = new DualShock4(this.runtime, Scratch3DualShock4Blocks.EXTENSION_ID);
  ...
  _createClass(Scratch3DualShock4Blocks, [{
    ...
    value: function getInfo() {
      return {
        ...
        // Test/Debug blocks
        {
          ...
          text: 'test DualShock 4 connection',
          ...
          opcode: 'showPairingHelp',
          text: 'show pairing help in console',
          blockType: BlockType.COMMAND
        }, '---',
        // Command blocks
        {
    ...
    value: function isButtonPressed(args) {
      ...
      return this._peripheral.isButtonPressed(button);
    }
    ...
    value: function testConnection() {
      console.log('🧪 Testing DualShock 4 connection...');
      if (this._peripheral.isConnected()) {
        console.log('✅ Controller is connected');
        // Try a simple vibration test
        this._peripheral.vibrate('both', 25, 0.5).then(function () {
    ...
    key: "showPairingHelp",
    value: function showPairingHelp() {
      console.log('🎮 DUALSHOCK 4 PAIRING HELP');
      console.log('==================================================');
      console.log('');
      console.log('📋 STEP 1: Pair in Windows Settings');
      console.log('   1. Open Windows Settings → Bluetooth & devices');
      console.log('   2. Click "Add device" → "Bluetooth"');
      console.log('   3. Turn OFF DualShock 4 completely');
      console.log('   4. Hold Share + PS buttons for 5 seconds (light flashes)');
      console.log('   5. Select "Wireless Controller" in Windows');
      console.log('   6. Wait for "Connected" status');
      console.log('');
      console.log('📋 STEP 2: Use Extension');
      console.log('   1. Make sure Scratch Link is running');
      console.log('   2. Press PS button to wake controller');
      console.log('   3. Click connection button in extension');
      console.log('   4. Select "Wireless Controller"');
      console.log('');
      console.log('🔧 TROUBLESHOOTING:');
      console.log('   • Controller already connected to PS4/phone? Disconnect it');
      console.log('   • Extension times out? Controller probably not paired');
      console.log('   • "Something failed"? Try pairing again');
      console.log('   • Still not working? Restart Scratch Link');
      console.log('');
      console.log('✅ Your controller address: DD:3B:5B:C0:F0:23');
    }
  }, {
    key: "getAxisValue",
    value: function getAxisValue(args) {
      ...
      return this._peripheral.getStickValue(stick, axis);
    }
  }, {
    key: "getTriggerValue",
    value: function getTriggerValue(args) {
      var trigger = Cast.toString(args.TRIGGER);
      return this._peripheral.getTriggerValue(trigger);
    }
  }, {
    key: "vibrate",
    value: function vibrate(args) {
      var motor = Cast.toString(args.MOTOR);
      var intensity = Cast.toNumber(args.INTENSITY);
      var duration = Cast.toNumber(args.DURATION);
      return this._peripheral.vibrate(motor, intensity, duration);
    }
  }, {
    key: "setLightBarColor",
