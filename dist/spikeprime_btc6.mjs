...
var SpikePrime = /*#__PURE__*/function () {
  function SpikePrime(runtime, extensionId) {
    ...
    this._sensors = {
      ...
      hubStatus: {},
      motorPositions: {},
      // Store motor positions from Anton's methods
      buttonTiming: {}
    };
    this._portValues = {};
  ...
  _createClass(SpikePrime, [{
    ...
    value: function sendPythonCommand(pythonCode) {
      ...
      return this.sendCommand('scratch.python_eval', {
        ...
        // If direct Python execution isn't available, mark as unavailable
        _this2._pythonAvailable = false;
    ...
    value: function _onConnect() {
      ...
      this.sendPythonCommand('import hub; print("PYTHON_AVAILABLE")').then(function () {
        _this3._pythonAvailable = true;
        console.log('Python execution available on hub');

        // FIXED: Initialize continuous sensor monitoring
        _this3._initializeContinuousSensorMonitoring();
      }).catch(function () {
    ...
    // FIXED: Properly continuous sensor monitoring using Anton's methods
  }, {
    ...
    value: function _initializeContinuousSensorMonitoring() {
      ...
      // Set up a continuous sensor monitoring loop on the hub
      var sensorScript = "\nimport hub\nimport utime\n\ndef continuous_sensor_loop():\n    while True:\n        try:\n            # Get gyro rates (Anton's method)\n            yaw_rate, pitch_rate, roll_rate = hub.motion.gyroscope()\n            \n            # Get filtered gyro rates (Anton's method)\n            yaw_rate_f, pitch_rate_f, roll_rate_f = hub.motion.gyroscope_filter()\n            \n            # Get angles (Anton's method)  \n            yaw_angle, pitch_angle, roll_angle = hub.motion.position()\n            \n            # Get acceleration (Anton's method)\n            accel_x, accel_y, accel_z = hub.motion.accelerometer()\n            \n            # Get filtered acceleration (Anton's method)\n            accel_x_f, accel_y_f, accel_z_f = hub.motion.accelerometer_filter()\n            \n            # Get temperatures (Anton's method)\n            try:\n                battery_temp = hub.battery.temperature()\n                hub_temp = hub.temperature()\n            except:\n                battery_temp = 25\n                hub_temp = 25\n                \n            # Get motor positions for all ports\n            motor_data = {}\n            for port in 'ABCDEF':\n                try:\n                    if hasattr(hub.port[port], 'motor'):\n                        speed, rel_deg, abs_deg, pwm = hub.port[port].motor.get()\n                        motor_data[port] = f\"{speed},{rel_deg},{abs_deg},{pwm}\"\n                except:\n                    pass\n                    \n            # Print all sensor data in one line for efficient parsing\n            motor_str = \"|\".join([f\"{k}:{v}\" for k, v in motor_data.items()])\n            print(f\"SENSORS:{yaw_rate},{pitch_rate},{roll_rate}|{yaw_rate_f},{pitch_rate_f},{roll_rate_f}|{yaw_angle},{pitch_angle},{roll_angle}|{accel_x},{accel_y},{accel_z}|{accel_x_f},{accel_y_f},{accel_z_f}|{battery_temp},{hub_temp}|{motor_str}\")\n            \n            # Check for gestures\n            for gesture in ['DOUBLETAPPED', 'FREEFALL', 'SHAKE', 'TAPPED']:\n                if hub.motion.was_gesture(getattr(hub.motion, gesture)):\n                    print(f\"GESTURE:{gesture}\")\n                    \n            # Get orientation\n            orientation = hub.motion.orientation()\n            print(f\"ORIENTATION:{orientation}\")\n            \n        except Exception as e:\n            print(f\"SENSOR_ERROR:{e}\")\n            \n        # Wait 100ms between readings for good performance\n        utime.sleep_ms(100)\n\n# Start the continuous loop\ncontinuous_sensor_loop()\n";
        ...
        for (_iterator.s(); !(_step = _iterator.n()).done;) {
          ...
          try {
            ...
            if (jsonText.includes(':')) {
              this._parseAntonData(jsonText.trim());
            } else {
    ...
    // FIXED: Comprehensive parsing of Anton's sensor data
  }, {
    key: "_parseAntonData",
    value: function _parseAntonData(dataText) {
      var _this4 = this;
      try {
        if (dataText.startsWith('SENSORS:')) {
          // Parse the comprehensive sensor data
          var sensorData = dataText.substring(8);
          var parts = sensorData.split('|');
          if (parts.length >= 6) {
            // Gyro rates
            var gyro = parts[0].split(',').map(parseFloat);
            if (gyro.length === 3) {
              this._sensors.gyro = {
                x: gyro[1],
                y: gyro[0],
            ...
            // Filtered gyro rates  
            var gyroFiltered = parts[1].split(',').map(parseFloat);
            if (gyroFiltered.length === 3) {
              this._sensors.gyroFiltered = {
                x: gyroFiltered[1],
                y: gyroFiltered[0],
                z: gyroFiltered[2]
              };
            ...
            // Angles
            var angles = parts[2].split(',').map(parseFloat);
            if (angles.length === 3) {
              this._sensors.angle = {
                yaw: angles[0],
                pitch: angles[1],
            ...
            // Acceleration
            var accel = parts[3].split(',').map(parseFloat);
            if (accel.length === 3) {
            ...
            var accelFiltered = parts[4].split(',').map(parseFloat);
            if (accelFiltered.length === 3) {
              this._sensors.accelerationFiltered = {
                x: accelFiltered[0],
                y: accelFiltered[1],
                z: accelFiltered[2]
              };
            ...
            // Temperatures
            var temps = parts[5].split(',').map(parseFloat);
            if (temps.length >= 2) {
              this._sensors.temperature = temps[0];
              this._sensors.hubTemp = temps[1];
            }

            // Motor positions (if present)
            if (parts.length > 6 && parts[6]) {
              var motorPairs = parts[6].split('|');
          ...
          if (this._sensors.gestures.hasOwnProperty(gesture)) {
            ...
            // Reset gesture after 100ms to make it edge-triggered
            setTimeout(function () {
              _this4._sensors.gestures[gesture] = false;
            }, 100);
          }
        } else if (dataText.startsWith('ORIENTATION:')) {
          var orientation = dataText.substring(12);
          var orientationMap = {
            'front': SpikeOrientation.front,
            'back': SpikeOrientation.back,
            'up': SpikeOrientation.up,
            'down': SpikeOrientation.down,
            'rightside': SpikeOrientation.rightside,
            'leftside': SpikeOrientation.leftside
          };
          if (orientationMap[orientation]) {
            this._sensors.orientation = orientationMap[orientation];
          }
        } else if (dataText.startsWith('BUTTON_TIME_')) {
          // Parse button timing from Anton's callback
          var _parts = dataText.split(':');
          if (_parts.length === 2) {
            var button = _parts[0].substring(12); // Remove 'BUTTON_TIME_'
            var timeMs = parseInt(_parts[1]);
            this._sensors.buttonTiming[button] = timeMs;
          }
        } else if (dataText.startsWith('PYTHON_AVAILABLE')) {
          this._pythonAvailable = true;
        }
      } catch (error) {
        console.warn('Error parsing Anton data:', error, 'Data:', dataText);
      }
    }
  }, {
    key: "_parseResponse",
  ...
  _createClass(Scratch3SpikePrimeBlocks, [{
    ...
    value: function _motorRunForDegrees(ports, direction, degrees) {
      ...
      var promises = ports.map(function (port) {
        ...
        var antonCommand = _this6._peripheral.sendPythonCommand("import hub; hub.port.".concat(port, ".motor.run_for_degrees(").concat(Math.floor(degrees), ", ").concat(setting.speed * direction, ")"));
    ...
    value: function _motorRunTimed(ports, direction, seconds) {
      ...
      var promises = ports.map(function (port) {
        ...
        var antonCommand = _this7._peripheral.sendPythonCommand("import hub; hub.port.".concat(port, ".motor.run_for_time(").concat(Math.floor(seconds * 1000), ", ").concat(setting.speed * direction, ")"));
        return standardCommand.catch(function () {
          return antonCommand;
        });
      });
    ...
    value: function motorStart(args) {
      ...
      var promises = ports.map(function (port) {
        ...
        // FIXED: Removed incorrect *10 scaling for PWM
        var antonCommand = _this8._peripheral.sendPythonCommand("import hub; hub.port.".concat(port, ".motor.pwm(").concat(Math.round(setting.speed * direction), ")"));
        return standardCommand.catch(function () {
          return antonCommand;
        });
      });
    ...
    value: function motorStop(args) {
      ...
      var promises = ports.map(function (port) {
        ...
        var antonCommand = _this9._peripheral.sendPythonCommand("import hub; hub.port.".concat(port, ".motor.stop()"));
        return standardCommand.catch(function () {
          return antonCommand;
        });
      });
    ...
    // ===== ANTON'S MOTOR ENHANCEMENTS (FIXED - HIGH CONFIDENCE) =====
  }, {
    ...
    value: function getRelativePosition(args) {
      ...
      var antonData = this._peripheral._sensors.motorPositions[port];
      if (antonData) {
        return antonData.relativePosition;
      }
    ...
    value: function getAbsolutePosition(args) {
      ...
      var antonData = this._peripheral._sensors.motorPositions[port];
      if (antonData) {
        return antonData.absolutePosition;
      }
    ...
    value: function getSpeed(args) {
      ...
      // FIXED: Use Anton's speed data directly, not conversion
      var antonData = this._peripheral._sensors.motorPositions[port];
      if (antonData) {
        // Anton's data is already in the correct format
        return Math.round(antonData.speed * 9.3); // Convert to deg/s using Anton's factor
      }
    ...
    // ===== ANTON'S IMU & GYRO (FIXED - HIGH CONFIDENCE) =====
  }, {
    key: "getAngle",
