...
var Scratch3SpikePrimeBlocks = /*#__PURE__*/function (_BleBaseBlocks) {
  ...
  function Scratch3SpikePrimeBlocks(runtime) {
    ...
    var LWP3_SERVICE_UUID = '00001623-1212-efde-1623-785feabcd123';
    _this = _callSuper(this, Scratch3SpikePrimeBlocks, [new Hub(runtime, Scratch3SpikePrimeBlocks.EXTENSION_ID, LWP3_SERVICE_UUID)]);
    ...
    _this._hubType = 0x81; // Default to SPIKE Prime
    return _this;
  }

  // Proper LWP3 message builder with length encoding
  _createClass(Scratch3SpikePrimeBlocks, [{
    key: "_buildLwp3Message",
    value: function _buildLwp3Message(payload) {
      var totalLength = payload.length + 1;
      if (totalLength <= 127) {
        return Buffer.concat([Buffer.from([totalLength]), payload]);
      } else {
        // Extended length encoding for messages > 127 bytes
        var lsb = 0x80 | totalLength & 0x7F;
        var msb = totalLength >> 7 & 0xFF;
    ...
    value: function getInfo() {
      ...
      return {
        ...
        blocks: [].concat(_toConsumableArray(this.getBaseBlocks()), ['---',
        ...
        menus: {
          ...
          HUB_BUTTON: {
            ...
            items: [{
              text: formatMessage({
                id: 'legobluetooth.button.center',
                default: 'center'
              }),
              value: 'center'
            }, {
              text: formatMessage({
                id: 'legobluetooth.button.connect',
                default: 'connect'
              }),
              value: 'connect'
            }]
          ...
          CENTER_LED_COLOR: {
            ...
            items: [{
              text: formatMessage({
                id: 'legobluetooth.centerled.off',
                default: 'off'
              }),
              value: 'off'
            }, {
              text: formatMessage({
                id: 'legobluetooth.centerled.green',
                default: 'green'
              }),
              value: 'green'
            }, {
              text: formatMessage({
                id: 'legobluetooth.centerled.red',
                default: 'red'
              }),
              value: 'red'
            }, {
              text: formatMessage({
                id: 'legobluetooth.centerled.blue',
                default: 'blue'
              }),
              value: 'blue'
            }]
          ...
          COLOR_ID: {
            ...
            items: [{
              text: formatMessage({
                id: 'legobluetooth.color.red',
                default: 'red'
              }),
              value: 9
            }, {
              text: formatMessage({
                id: 'legobluetooth.color.green',
                default: 'green'
              }),
              value: 6
            }, {
              text: formatMessage({
                id: 'legobluetooth.color.blue',
                default: 'blue'
              }),
              value: 3
            }, {
              text: formatMessage({
                id: 'legobluetooth.color.yellow',
                default: 'yellow'
              }),
              value: 7
            }]
    ...
    value: function motorGoDirectionToPosition(args) {
      ...
      var promises = ports.map(function (port) {
        ...
        var speed = 75; // Default for 'shortest'
        ...
        var maxPower = 100;
        var endState = 0x7E; // HOLD (126)
        var useProfile = 0x01; // Use acceleration/deceleration profiles

        // Proper 13-byte command payload
        var commandPayload = Buffer.alloc(13);
        ...
        var finalMessage = _this2._buildLwp3Message(commandPayload);
        return _this2._peripheral.write(finalMessage);
      });
    ...
    value: function resetMotorPosition(args) {
      ...
      var portId = this.externalPorts.indexOf(portName);
      if (portId === -1) return Promise.resolve();
      var newPosition = Math.round(Cast.toNumber(args.POSITION));

      // Proper preset encoder command using WriteDirectModeData
      var commandPayload = Buffer.alloc(10);
      commandPayload.writeUInt8(0x00, 0); // Hub ID
      commandPayload.writeUInt8(0x81, 1); // Port Output Command
      commandPayload.writeUInt8(portId, 2); // Port ID
      ...
      var finalMessage = this._buildLwp3Message(commandPayload);
    ...
    value: function getDistanceValue(args) {
      ...
      var distanceRaw = this._peripheral.inputValue(portId, 'distance') || 0;
    ...
    value: function displayImage(args) {
      var imagePayload = this._get5x5MatrixPayload(args.MATRIX);

      // Use Hub Properties to control internal display
      var commandPayload = Buffer.concat([Buffer.from([0x00,
      // Hub ID
      0x81,
      // Port Output Command
      0x32,
      // Internal display port (hub display)
      0x11,
      // Startup/Completion Info
      0x51,
      // WriteDirectModeData
    ...
    value: function displayText(args) {
      ...
      var textBytes = Buffer.from(text, 'utf8').slice(0, 20);
      var commandPayload = Buffer.concat([Buffer.from([0x00,
      // Hub ID
      0x81,
      // Port Output Command
    ...
    value: function displayClear() {
      var payload = Buffer.alloc(25); // 25 zero bytes
    ...
    value: function setMatrixLED(args) {
      ...
      var byteValue = colorId === 0 ? 0 : brightness << 4 | colorId;
    ...
    value: function setMatrixAll(args) {
      ...
      if (colorId === 0) {
        ...
        var commandPayload = Buffer.from([0x00,
        // Hub ID
        0x81,
        // Port Output Command
        portId,
        ...
        0x51,
        // WriteDirectModeData
        SensorMode.MATRIX.COLOR_OUTPUT,
        // Mode 1: All LEDs same color
        colorId]);

        // Update state tracking
        var byteValue = brightness << 4 | colorId;
    ...
    key: "getHubBatteryLevel",
    value: function getHubBatteryLevel() {
    ...
    key: "getHubFirmwareVersion",
    value: function getHubFirmwareVersion() {
    ...
    key: "whenHubButtonPressed",
    value: function whenHubButtonPressed(args) {
      return this.isHubButtonPressed(args);
    }
  }, {
    key: "isHubButtonPressed",
    value: function isHubButtonPressed(args) {
      Cast.toString(args.BUTTON);
    ...
    key: "setCenterButtonLED",
    value: function setCenterButtonLED(args) {
    ...
    key: "playBeepTone",
    value: function playBeepTone(args) {
      var frequency = MathUtil.clamp(Cast.toNumber(args.FREQUENCY), 20, 20000);
      var duration = MathUtil.clamp(Cast.toNumber(args.DURATION), 1, 10000);

      // Use Hub Actions command for beep
      var commandPayload = Buffer.alloc(6);
      commandPayload.writeUInt8(0x00, 0); // Hub ID
    ...
    value: function _get5x5MatrixPayload(matrixString) {
      ...
      var symbol = (Cast.toString(matrixString).replace(/[^01]/g, '') + '0'.repeat(25)).slice(0, 25);
      var payload = [];
      for (var i = 0; i < 25; i++) {
    ...
    key: "_subscribeToPortValue",
    value: function _subscribeToPortValue(portId, mode) {
    ...
    key: "getBaseBlocks",
    value: function getBaseBlocks() {
      try {
        return _get(_getPrototypeOf(Scratch3SpikePrimeBlocks.prototype), "getBlocks", this) ? _get(_getPrototypeOf(Scratch3SpikePrimeBlocks.prototype), "getBlocks", this).call(this, formatMessage) : [];
      } catch (error) {
