...
/**
 * A class to manage the Bluetooth Classic connection and communication protocol for SPIKE Prime.
 */
var SpikePrimeBTC = /*#__PURE__*/function () {
  function SpikePrimeBTC(runtime, extensionId) {
    _classCallCheck(this, SpikePrimeBTC);
    this._runtime = runtime;
    this._extensionId = extensionId;
    this._bt = null;

    // Register this extension as a peripheral extension
  ...
  _createClass(SpikePrimeBTC, [{
    ...
    value: function scan() {
      ...
      // Scan for ANY Bluetooth Classic device offering a Serial Port.
      // Remove device class filters to let user select from the full list.
      // Based on our debug scan, the hub appears with class 0x002804
      this._bt = new BT(this._runtime, this._extensionId, {},
      // No filters - let user choose from all available devices
    ...
    key: "isConnected",
    value: function isConnected() {
      return this._bt && this._bt.isConnected();
    }

    /**
     * CORRECTED: Sends a JSON command, encoding it in Base64 as the BT class expects.
     * @param {object} json - The JSON command object
    ...
    value: function sendJSON(json) {
      ...
      // Encode message as Base64 - this is required by the BT class
      var messageBase64 = Base64Util.uint8ArrayToBase64(new TextEncoder().encode(jsonText + '\r'));

      // This is the required format for the bt.js sendMessage method
    ...
    /**
     * Send a command to the SPIKE Prime hub
    ...
    value: function sendCommand(method, params) {
      ...
      // The old firmware protocol does not use request IDs ('i'), so we send simple notifications.
    ...
    value: function _onConnect() {
      var _this = this;
      console.log('SPIKE Prime Hub connected via Bluetooth Classic');

      // Give the hub a moment to settle after connection
      setTimeout(function () {
        // Ask the hub to start sending sensor data
        _this.sendCommand('trigger_current_state', {});
      }, 500);
    }

    /**
     * CORRECTED: Handles incoming messages by decoding the Base64 payload from the params object.
     */
  }, {
    key: "_onMessage",
    value: function _onMessage(params) {
      ...
      // The data from the hub is in params.message and is Base64 encoded by Scratch Link
      var messageBase64 = params.message;
      var data = Base64Util.base64ToUint8Array(messageBase64);
      var text = new TextDecoder().decode(data);

      // Accumulate data for complete JSON messages
      this._pendingData += text;

      // Split by carriage return to get complete messages
      var messages = this._pendingData.split('\r');

      // Keep any incomplete message for next time
        ...
        for (_iterator.s(); !(_step = _iterator.n()).done;) {
          ...
          if (jsonText.trim()) {
            try {
              ...
              console.warn('Received invalid JSON:', jsonText);
            }
          }
        }
      } catch (err) {
    ...
    value: function _parseResponse(response) {
      ...
      if (response.hasOwnProperty('m')) {
        switch (response.m) {
          case 0:
            // Hub status (Ports, Acceleration, Gyro Rate, Tilt Angle, etc.)
            if (response.p && Array.isArray(response.p)) {
              // Parse port data (first 6 elements are ports A-F)
              for (var i = 0; i < Math.min(6, response.p.length); i++) {
                var port = SpikePorts[i];
                var portData = response.p[i];
                if (Array.isArray(portData) && portData.length >= 2) {
                  var deviceId = portData[0];
                  var values = portData[1];
                  switch (deviceId) {
                    case 48: // Large motor
                    case 49:
                      // Medium motor
                      if (Array.isArray(values) && values.length >= 4) {
                        this._portValues[port] = {
                          ...
                          position: values[2] ? (values[2] + 360) % 360 : 0,
                          power: values[3] || 0
                        };
                      }
                      break;
              ...
              // Parse tilt angle data (usually at index 8)
              if (response.p.length > 8 && Array.isArray(response.p[8])) {
          ...
          case 3:
            ...
            break;
          case 4:
            // Orientation/Gesture event
            if (typeof response.p === 'string' && SpikeOrientation.hasOwnProperty(response.p)) {
    ...
    value: function stopAll() {
      // Stop all motors and sounds
      this.sendCommand('program_terminate', {});
    }
  }, {
    key: "stopSound",
    value: function stopSound() {
      this.sendCommand('scratch.sound_stop', {});
    }
  }, {
    key: "stopAllMotors",
    value: function stopAllMotors() {
      // Stop all motors on all ports
      for (var _i = 0, _SpikePorts = SpikePorts; _i < _SpikePorts.length; _i++) {
        var port = _SpikePorts[_i];
  ...
  return SpikePrimeBTC;
}();
/**
 * The main Scratch extension class for SPIKE Prime Bluetooth Classic
 */
var Scratch3SpikePrimeBTCBlocks = /*#__PURE__*/function () {
  function Scratch3SpikePrimeBTCBlocks(runtime) {
    _classCallCheck(this, Scratch3SpikePrimeBTCBlocks);
    this.runtime = runtime;
    this._peripheral = new SpikePrimeBTC(runtime, Scratch3SpikePrimeBTCBlocks.EXTENSION_ID);
    if (runtime.formatMessage) {
      formatMessage = runtime.formatMessage;
    }
  }
  _createClass(Scratch3SpikePrimeBTCBlocks, [{
    key: "getInfo",
    value: function getInfo() {
      return {
        id: Scratch3SpikePrimeBTCBlocks.EXTENSION_ID,
        name: 'SPIKE Prime (Bluetooth Classic)',
        blockIconURI: blockIconURI,
        showStatusButton: true,
        blocks: [{
          opcode: 'motorRunFor',
          text: formatMessage({
            id: 'spikeprimeBTC.motorRunFor',
          ...
          text: formatMessage({
            id: 'spikeprimeBTC.motorSetSpeed',
        ...
        menus: {
          ...
          MOTOR_UNIT: {
            ...
            items: [{
              text: formatMessage({
                id: 'spikeprimeBTC.rotations',
                default: 'rotations'
              }),
              value: 'rotations'
            }, {
              text: formatMessage({
                id: 'spikeprimeBTC.degrees',
                default: 'degrees'
              }),
              value: 'degrees'
            }, {
              text: formatMessage({
                id: 'spikeprimeBTC.seconds',
                default: 'seconds'
              }),
              value: 'seconds'
            }]
          ...
          AXIS: {
            ...
            items: [{
              text: formatMessage({
                id: 'spikeprimeBTC.pitch',
                default: 'pitch'
              }),
              value: 'pitch'
            }, {
              text: formatMessage({
                id: 'spikeprimeBTC.roll',
                default: 'roll'
              }),
              value: 'roll'
            }, {
              text: formatMessage({
                id: 'spikeprimeBTC.yaw',
                default: 'yaw'
              }),
              value: 'yaw'
            }]
    ...
    value: function motorStart(args) {
      ...
      var promises = ports.map(function (port) {
        ...
        return _this4._peripheral.sendCommand('scratch.motor_start', {
          port: port,
          speed: setting.speed * direction,
          stall: setting.stallDetection
        });
    ...
    value: function motorStop(args) {
      ...
      var promises = ports.map(function (port) {
        ...
        return _this5._peripheral.sendCommand('scratch.motor_stop', {
          port: port,
          stop: setting.stopMode
        });
      });
    ...
    value: function motorSetSpeed(args) {
      ...
      ports.forEach(function (port) {
        _this6._peripheral.motorSettings[port].speed = speed;
      });
    }
  }, {
    key: "getPosition",
  ...
  return Scratch3SpikePrimeBTCBlocks;
}();
var blockClass = Scratch3SpikePrimeBTCBlocks;
blockClass = Scratch3SpikePrimeBTCBlocks;
