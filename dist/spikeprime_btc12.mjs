    ...
    if (typeof Blockly !== 'undefined' && Blockly.Field) {
      ...
      var FieldMatrix3x3Color = /*#__PURE__*/function (_Blockly$Field) {
        ...
        _createClass(FieldMatrix3x3Color, [{
          ...
          function initView() {
            ...
            this.textElement_.style.display = 'none'; // Hide text, show only matrix
          }
          ...
          value: function updateSize_() {
            this.movableGroup_.getBBox();
            var width = 90; // 3x3 grid at 30px per cell
            var height = 90;
            if (this.borderRect_) {
              width += this.constants_.FIELD_BORDER_RECT_X_PADDING * 2;
              height += this.constants_.FIELD_BORDER_RECT_Y_PADDING * 2;
          ...
          value: function createMatrixDisplay_() {
            ...
            // Create 3x3 grid background with rounded corners
            ...
            // Create 9 clickable cells arranged in 3x3 grid
            this.cells_ = [];
          ...
          value: function doValueUpdate_(newValue) {
            if (!newValue) newValue = '000000000000000000000000000'; // Default: all LEDs off

            // Parse encoded value: each 3 chars = colorId + brightness + reserved
            for (var i = 0; i < 9; i++) {
              var offset = i * 3;
      ...
      var registerField = function registerField() {
        try {
          ...
          console.warn('Matrix 3x3 field registration failed:', error);
        }
      };

      // Try immediate registration, then with delay if needed
      if (Blockly.fieldRegistry || Blockly.Field && Blockly.Field.register) {
        registerField();
      } else {
        setTimeout(registerField, 100);
      }

      // BLOCK DEFINITION FOR 3X3 MATRIX PATTERN DESIGNER
      // Creates a value block that returns the encoded pattern string
      var defineBlock = function defineBlock() {
        if (Blockly.Blocks) {
          Blockly.Blocks['matrix_3x3_color'] = {
            init: function init() {
              this.jsonInit({
                "message0": "%1",
                "args0": [{
                ...
                "outputShape": Blockly.OUTPUT_SHAPE_ROUND || 1,
                "output": "String",
                "colour": "#9966FF",
                "colourSecondary": "#7700CC",
                "colourTertiary": "#550099"
              });
            }
          };
        }
      ...
      if (Blockly.Blocks) {
        ...
        setTimeout(defineBlock, 100);
      }
    }
    // Placing the definition above within the constructor should ensured the custom field is registered only when the extension is loaded
    // and the Blockly environment is available.
