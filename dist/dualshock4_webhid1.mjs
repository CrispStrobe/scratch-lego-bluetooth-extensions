...
// Button mapping based on node-dualshock-controller and standard HID reports
var DS4_BUTTONS = {
  SQUARE: 0,
  X: 1,
  CIRCLE: 2,
...
var DualShock4_HID_Peripheral = /*#__PURE__*/function () {
  function DualShock4_HID_Peripheral() {
  ...
  _createClass(DualShock4_HID_Peripheral, [{
      ...
      var _connect = _asyncToGenerator( /*#__PURE__*/regenerator.mark(function _callee() {
        ...
        return regenerator.wrap(function _callee$(_context) {
          while (1) switch (_context.prev = _context.next) {
            ...
            case 13:
              console.log('🎮 DualShock 4 Connected via WebHID:', this.device.productName);
    ...
    value: function _onInputReport(event) {
      ...
      // Standard DS4 report ID is 1. We check for a reasonable data length.
      if (reportId !== 1 || data.byteLength < 8) return;

      // Save previous state for HAT block logic
      this._previousButtons = _toConsumableArray(this.state.buttons);

      // --- Parse Sticks & Triggers ---
      this.state.sticks.left.x = data.getUint8(0) / 127.5 - 1;
      this.state.sticks.left.y = data.getUint8(1) / 127.5 - 1;
      this.state.sticks.right.x = data.getUint8(2) / 127.5 - 1;
      this.state.sticks.right.y = data.getUint8(3) / 127.5 - 1;
      this.state.triggers.l2 = data.getUint8(6) / 255;
      this.state.triggers.r2 = data.getUint8(7) / 255;

      // --- Parse Buttons ---
      var buttonByte5 = data.getUint8(4);
      var buttonByte6 = data.getUint8(5);

      // D-Pad (is a 4-bit value, 8 means neutral)
      var dpad = buttonByte5 & 0x0F;
      ...
      // Face Buttons
      this.state.buttons[DS4_BUTTONS.SQUARE] = (buttonByte5 & 0x10) !== 0;
      this.state.buttons[DS4_BUTTONS.X] = (buttonByte5 & 0x20) !== 0;
      this.state.buttons[DS4_BUTTONS.CIRCLE] = (buttonByte5 & 0x40) !== 0;
      this.state.buttons[DS4_BUTTONS.TRIANGLE] = (buttonByte5 & 0x80) !== 0;

      // Shoulder Buttons & Sticks
      this.state.buttons[DS4_BUTTONS.L1] = (buttonByte6 & 0x01) !== 0;
      this.state.buttons[DS4_BUTTONS.R1] = (buttonByte6 & 0x02) !== 0;
      this.state.buttons[DS4_BUTTONS.L2] = (buttonByte6 & 0x04) !== 0;
      this.state.buttons[DS4_BUTTONS.R2] = (buttonByte6 & 0x08) !== 0;
      this.state.buttons[DS4_BUTTONS.SHARE] = (buttonByte6 & 0x10) !== 0;
      this.state.buttons[DS4_BUTTONS.OPTIONS] = (buttonByte6 & 0x20) !== 0;
      this.state.buttons[DS4_BUTTONS.L3] = (buttonByte6 & 0x40) !== 0;
      this.state.buttons[DS4_BUTTONS.R3] = (buttonByte6 & 0x80) !== 0;
    }
  }]);
...
var Scratch3DualShock4Blocks = /*#__PURE__*/function () {
  function Scratch3DualShock4Blocks(runtime) {
    ...
    this._peripheral = new DualShock4_HID_Peripheral();
  }
  _createClass(Scratch3DualShock4Blocks, [{
    ...
    value: function whenButtonPressed(args) {
      ...
      // Trigger HAT block only on the rising edge (when it was false and now is true)
      return this._peripheral.state.buttons[buttonIndex] && !this._peripheral._previousButtons[buttonIndex];
    }
  }, {
    key: "isButtonPressed",
    ...
    value: function getAxisValue(args) {
      ...
      var stick = this._peripheral.state.sticks[args.STICK];
      if (!stick) return 0;
      var value = stick[args.AXIS];
      // Apply deadzone and scale to -100 to 100
