...
var Scratch3SpikePrimeBlocks = /*#__PURE__*/function () {
  function Scratch3SpikePrimeBlocks(runtime) {
    ...
    if (!Scratch3SpikePrimeBlocks.fieldRegistered) {
      console.log('[SPIKE Prime] Attempting to register custom 3x3 color matrix field...');

      // ===== 3X3 COLOR MATRIX FIELD IMPLEMENTATION FOR XCRATCH =====
      /**
       * Modern implementation of a 3x3 LED color matrix field for LEGO devices.
       * Uses the current Blockly field registration API for xcratch compatibility.
       * 
       * This field allows users to design 3x3 patterns with colors and brightness
       * levels that map to LEGO device LED matrices like those found on SPIKE Prime
       * 3x3 LED matrix accessories.
       */
      if (typeof Blockly !== 'undefined' && Blockly.Field) {
        /**
         * Class for 3x3 color matrix field - a clickable 3x3 grid with color picker
         * Based on Blockly.Field but with specialized UI for LEGO 3x3 LED matrices
         * 
         * Features:
         * - 3x3 grid of clickable LED cells
         * - 11 predefined LEGO-compatible colors
         * - Brightness control (1-10 levels)
         * - Interactive color palette dropdown
         * - Encoded value storage for efficient data transfer
         */
        var FieldMatrix3x3Color = /*#__PURE__*/function (_Blockly$Field) {
          _inherits(FieldMatrix3x3Color, _Blockly$Field);
          /**
           * Constructor for the 3x3 color matrix field
           * @param {string} value - Encoded matrix pattern (27 characters)
           * @param {Function} validator - Optional validation function
           */
          function FieldMatrix3x3Color(value, validator) {
            var _this6;
            _classCallCheck(this, FieldMatrix3x3Color);
            _this6 = _callSuper(this, FieldMatrix3x3Color, [value, validator]);
            console.log("[FieldMatrix3x3Color] Constructor called with value: \"".concat(value, "\""));

            // LEGO 3x3 matrix colors (protocol defined by LEGO firmware)
            _this6.colors_ = ['#222222',
            // 0: off (dark gray)
            '#FF00FF',
            // 1: magenta
            '#8000FF',
            ...
            _this6.colorNames_ = ['off', 'magenta', 'violet', 'blue', 'turquoise', 'mint', 'green', 'yellow', 'orange', 'red', 'white'];

            // Current matrix state: 9 pixels, each with colorId + brightness
            _this6.matrix_ = Array(9).fill().map(function () {
              return {
                colorId: 0,
                brightness: 5
              };
            ...
            // UI state for interactive editing
            _this6.selectedColor_ = 9; // Default to red
            _this6.selectedBrightness_ = 5; // Default brightness (1-10 scale)
            _this6.isDropdownVisible_ = false;

            // Set default heart pattern if no value provided
            _this6.setValue(value || '960000960960a60960960000960');
            return _this6;
          }

          /**
           * Static factory method required by modern Blockly field registration.
           * This method is called when creating fields from JSON block definitions.
           * 
           * @param {!Object} options - JSON options from block definition
           * @returns {!FieldMatrix3x3Color} New field instance
           */
          _createClass(FieldMatrix3x3Color, [{
            key: "initView",
            value:
            /**
             * Initialize the field's visual representation
             * Called automatically by Blockly during field creation
             */
            function initView() {
              console.log('[FieldMatrix3x3Color] initView()');
              this.createBorderRect_();
              if (this.borderRect_) {
                this.borderRect_.style['fill-opacity'] = 1;
              }
              this.createTextElement_();
              if (this.textElement_) {
                this.textElement_.style.display = 'none'; // Hide default text label
              }

              // Create the matrix display elements ONCE during initialization.
              this.createMatrixDisplay_();
            }

            /**
             * Render the field's visual elements
             * Called automatically when the field needs to be redrawn
             */
          }, {
            key: "render_",
            value: function render_() {
              // console.log('[FieldMatrix3x3Color] render_()'); // This can be very noisy
              if (!this.borderRect_ || !this.matrixGroup_) {
                console.warn('[FieldMatrix3x3Color] render_() called before initView() finished.');
                return;
              }
              this.borderRect_.setAttribute('width', this.size_.width);
              this.borderRect_.setAttribute('height', this.size_.height);

              // Update the colors of the matrix to reflect the current value.
              this.updateMatrix_();
            }

            /**
             * Calculate and update the field's size based on content
             * Called automatically during layout operations
             */
          }, {
            key: "updateSize_",
            value: function updateSize_() {
              var width = 90; // 3 cells * 25px + padding/margins
              var height = 90;
              if (this.borderRect_) {
                // Use fallbacks in case constants are not available in all versions
                var paddingX = this.constants_ && this.constants_.FIELD_BORDER_RECT_X_PADDING ? this.constants_.FIELD_BORDER_RECT_X_PADDING : 5;
                var paddingY = this.constants_ && this.constants_.FIELD_BORDER_RECT_Y_PADDING ? this.constants_.FIELD_BORDER_RECT_Y_PADDING : 2;
                this.size_.width = width + paddingX * 2;
                this.size_.height = height + paddingY * 2;
              } else {
                this.size_.width = width;
                this.size_.height = height;
              }
              console.log("[FieldMatrix3x3Color] updateSize_() to width: ".concat(this.size_.width, ", height: ").concat(this.size_.height));
            }

            /**
             * Create the 3x3 matrix display with clickable cells
             * Builds the SVG structure for the LED grid visualization
             */
          }, {
            key: "createMatrixDisplay_",
            value: function createMatrixDisplay_() {
              console.log('[FieldMatrix3x3Color] createMatrixDisplay_() building SVG elements.');
              this.matrixGroup_ = Blockly.utils.dom.createSvgElement('g', {}, this.fieldGroup_);
              this.matrixBg_ = Blockly.utils.dom.createSvgElement('rect', {
                x: 5,
                y: 5,
                width: 80,
                height: 80,
              ...
              for (var row = 0; row < 3; row++) {
                for (var col = 0; col < 3; col++) {
                  var index = row * 3 + col;
                  var cell = this.createCell_(col, row, index);
                  this.cells_.push(cell);
                  this.matrixGroup_.appendChild(cell);
                }
              }
            }

            /**
             * Create a single LED cell element
             * @param {number} col - Column position (0-2)
             * @param {number} row - Row position (0-2)  
             * @param {number} index - Linear index (0-8)
             * @returns {SVGElement} Cell group element
             */
          }, {
            key: "createCell_",
            value: function createCell_(col, row, index) {
              var x = 8 + col * 25;
              var y = 8 + row * 25;
              var cellGroup = Blockly.utils.dom.createSvgElement('g', {}, null);
              var rect = Blockly.utils.dom.createSvgElement('rect', {
                ...
                rx: 2,
                'data-index': index,
                class: 'matrix3x3-cell'
              }, cellGroup);

              // Bind click event for cell interaction
              this.bindEvents_(rect);
              return cellGroup;
            }

            /**
             * Bind mouse events to cell elements
             * @param {SVGElement} element - Element to bind events to
             */
          }, {
            key: "bindEvents_",
            value: function bindEvents_(element) {
              this.mouseDownWrapper_ = Blockly.bindEventWithChecks_(element, 'mousedown', this, this.onMouseDown_);
            }

            /**
             * Handle mouse click on matrix cells
             * First click opens color picker, subsequent clicks set colors
            ...
            value: function onMouseDown_(e) {
              ...
              if (!this.isDropdownVisible_) {
                this.showDropdown_();
              } else {
                this.setCellColor_(index, this.selectedColor_, this.selectedBrightness_);
                this.updateMatrix_();
                this.setValue(this.getEncodedValue_());
              }
              e.stopPropagation();
            }

            /**
             * Show the color picker dropdown interface
             * Creates an interactive palette for color and brightness selection
             */
          }, {
            key: "showDropdown_",
            value: function showDropdown_() {
              if (this.isDropdownVisible_) return;
              this.isDropdownVisible_ = true;
              this.dropdownGroup_ = this.createDropdown_();
              this.matrixGroup_.appendChild(this.dropdownGroup_);
            }
            ...
            key: "hideDropdown_",
            value: function hideDropdown_() {
              if (!this.isDropdownVisible_) return;
              this.isDropdownVisible_ = false;
              if (this.dropdownGroup_) {
                this.dropdownGroup_.remove();
                this.dropdownGroup_ = null;
              }
            }

            /**
             * Create the color picker dropdown interface
             * Includes color buttons, brightness slider, and close button
             * @returns {SVGElement} Dropdown group element
             */
          }, {
            key: "createDropdown_",
            value: function createDropdown_() {
              var _this7 = this;
              var dropdown = Blockly.utils.dom.createSvgElement('g', {}, null);
              ...
              var closeBtn = Blockly.utils.dom.createSvgElement('rect', {
                x: 250,
                y: 5,
                width: 15,
                height: 15,
              ...
              var closeX = Blockly.utils.dom.createSvgElement('text', {
                x: 257.5,
                y: 15,
                'text-anchor': 'middle',
                'font-size': '10',
              ...
              closeX.textContent = '×';
              Blockly.bindEventWithChecks_(closeBtn, 'mousedown', this, function () {
                _this7.hideDropdown_();
              });

              // Color palette layout: 3 rows × 4 columns
              // Organized by color families for intuitive selection
              var colorLayout = [[0, 9, 8, 7],
              // off, red, orange, yellow
              [6, 5, 4, 3],
              // green, mint, turquoise, blue  
              [2, 1, 10, -1] // violet, magenta, white, brightness control
              ];
              for (var row = 0; row < 3; row++) {
                for (var col = 0; col < 4; col++) {
                  var colorId = colorLayout[row][col];
                  var x = 95 + col * 30;
                  var y = 10 + row * 25;
                  if (colorId === -1) {
                    // Special case: brightness control slider
                    this.createBrightnessControl_(dropdown, x, y);
                  } else {
                    // Regular color selection button
                    this.createColorButton_(dropdown, colorId, x, y);
                  }
                }
              }
              return dropdown;
             ...
             * @param {number} colorId - Color ID (0-10)
             * @param {number} x - X position
             * @param {number} y - Y position
             * @returns {SVGElement} Button element
             */
          }, {
            key: "createColorButton_",
            value: function createColorButton_(parent, colorId, x, y) {
              var _this8 = this;
              var btn = Blockly.utils.dom.createSvgElement('rect', {
                x: x,
                y: y,
                width: 25,
                height: 20,
                fill: this.colors_[colorId],
                stroke: '#666',
                'stroke-width': 1,
                rx: 2,
                'data-color': colorId
              ...
              // Special visual styling for "off" state
              if (colorId === 0) {
                btn.setAttribute('stroke-dasharray', '2,2');
              }
              Blockly.bindEventWithChecks_(btn, 'mousedown', this, function (e) {
                _this8.selectedColor_ = colorId;
                _this8.updateColorButtons_();
                e.stopPropagation();
              });
              return btn;
            }
             ...
             * Interactive vertical slider for brightness selection (1-10)
             * @param {SVGElement} parent - Parent element
             * @param {number} x - X position
             * @param {number} y - Y position
             */
          }, {
            key: "createBrightnessControl_",
            value: function createBrightnessControl_(parent, x, y) {
              var _this9 = this;
              // Background container
              var bg = Blockly.utils.dom.createSvgElement('rect', {
                x: x,
                y: y,
                width: 25,
                height: 20,
              ...
              this.brightnessBar_ = Blockly.utils.dom.createSvgElement('rect', {
                x: x + 2,
                y: y + 2 + (16 - this.selectedBrightness_ * 1.6),
                width: 21,
                height: this.selectedBrightness_ * 1.6,
                fill: '#ffd700'
              }, parent);

              // Numeric brightness display
              this.brightnessText_ = Blockly.utils.dom.createSvgElement('text', {
                x: x + 12.5,
                y: y + 13,
                'text-anchor': 'middle',
                'font-size': '8',
              ...
              this.brightnessText_.textContent = this.selectedBrightness_;

              // Handle brightness adjustment clicks
              Blockly.bindEventWithChecks_(bg, 'mousedown', this, function (e) {
                var rect = bg.getBoundingClientRect();
                var clickY = e.clientY - rect.top;
                _this9.selectedBrightness_ = Math.max(1, Math.min(10, Math.round((20 - clickY) / 2)));
                _this9.updateBrightnessControl_();
                e.stopPropagation();
              });
            }

            /**
             * Update visual state of color selection buttons
             * Highlights the currently selected color
             */
          }, {
            key: "updateColorButtons_",
            value: function updateColorButtons_() {
              var _this10 = this;
              if (!this.dropdownGroup_) return;
              var buttons = this.dropdownGroup_.querySelectorAll('[data-color]');
              buttons.forEach(function (btn) {
                var colorId = parseInt(btn.getAttribute('data-color'));
                if (colorId === _this10.selectedColor_) {
                  btn.setAttribute('stroke', '#00ff00');
            ...
            /**
             * Update brightness control visual display
             * Adjusts bar height and numeric display
             */
          }, {
            key: "updateBrightnessControl_",
            value: function updateBrightnessControl_() {
              if (this.brightnessBar_) {
                var currentY = parseFloat(this.brightnessBar_.getAttribute('y'));
                var baseY = currentY - parseFloat(this.brightnessBar_.getAttribute('height')) + 2;
                this.brightnessBar_.setAttribute('y', baseY + (16 - this.selectedBrightness_ * 1.6));
                this.brightnessBar_.setAttribute('height', this.selectedBrightness_ * 1.6);
              }
              if (this.brightnessText_) {
                this.brightnessText_.textContent = this.selectedBrightness_;
              }
            }

            /**
             * Set color and brightness for a specific cell
             * @param {number} index - Cell index (0-8)
             * @param {number} colorId - Color ID (0-10)
             * @param {number} brightness - Brightness level (1-10)
             */
          }, {
            key: "setCellColor_",
            value: function setCellColor_(index, colorId, brightness) {
              ...
              var rect = cell.querySelector('rect');
              if (rect) {
                var color = this.colors_[colorId];
                var opacity = colorId === 0 ? 0.3 : brightness / 10;
            ...
            /**
             * Update all matrix cells to reflect current data state
             */
          }, {
            key: "updateMatrix_",
            value: function updateMatrix_() {
              var _this11 = this;
              this.matrix_.forEach(function (pixel, index) {
                _this11.setCellColor_(index, pixel.colorId, pixel.brightness);
              });
            }

            /**
             * Encode matrix state as a compact string for storage and transmission
             * Format: 27 characters = 9 pixels × 3 chars per pixel (color+brightness+reserved)
             * @returns {string} Encoded matrix pattern
             */
          }, {
            key: "getEncodedValue_",
            value: function getEncodedValue_() {
              return this.matrix_.map(function (pixel) {
                var colorHex = pixel.colorId.toString(16);
                var brightnessHex = pixel.brightness.toString(16);
                return colorHex + brightnessHex + '0'; // Third char reserved for future use
              }).join('');
            }

            /**
             * Parse encoded string and update matrix state
             * @param {string} newValue - Encoded pattern string
             */
          }, {
            key: "doValueUpdate_",
            value: function doValueUpdate_(newValue) {
              console.log("[FieldMatrix3x3Color] doValueUpdate_ with value: \"".concat(newValue, "\""));
              _get(_getPrototypeOf(FieldMatrix3x3Color.prototype), "doValueUpdate_", this).call(this, newValue); // Important to call super!
              if (!newValue || newValue.length < 27) {
                console.warn('[FieldMatrix3x3Color] Invalid or short value received, defaulting to off.');
                newValue = '000000000000000000000000000';
              }
              for (var i = 0; i < 9; i++) {
                var offset = i * 3;
                var colorId = parseInt(newValue.charAt(offset), 16) || 0;
                var brightness = parseInt(newValue.charAt(offset + 1), 16) || 5;
                this.matrix_[i] = {
                  colorId: colorId,
                  brightness: brightness
                };
              }
              if (this.matrixGroup_) {
                this.updateMatrix_();
              }
            }

            /**
             * Load field value from XML (for workspace save/load)
             * @param {Element} fieldElement - XML element containing field data
             */
          }, {
            key: "fromXml",
            value: function fromXml(fieldElement) {
            ...
            value: function dispose() {
              ...
              _get(_getPrototypeOf(FieldMatrix3x3Color.prototype), "dispose", this).call(this);
            }
          }], [{
            key: "fromJson",
            value: function fromJson(options) {
              var value = Blockly.utils.parsing.replaceMessageReferences(options.value || options.defaultValue || '');
              console.log('[FieldMatrix3x3Color] fromJson creating new field with value:', value);
              return new FieldMatrix3x3Color(value);
            }
          }]);
          return FieldMatrix3x3Color;
        }(Blockly.Field); // REGISTRATION - using Blockly field registry API
        try {
          if (Blockly.fieldRegistry && Blockly.fieldRegistry.register) {
            Blockly.fieldRegistry.register('field_matrix_3x3_color', FieldMatrix3x3Color);
            console.log('✅ Successfully registered field_matrix_3x3_color using fieldRegistry.');
          } else if (Blockly.Field && Blockly.Field.register) {
            // Fallback for older versions
            Blockly.Field.register('field_matrix_3x3_color', FieldMatrix3x3Color);
            console.log('✅ Successfully registered field_matrix_3x3_color using Field.register (fallback).');
          } else {
            console.error('❌ Could not find a valid Blockly registration method.');
          }
          Scratch3SpikePrimeBlocks.fieldRegistered = true;
        } catch (error) {
          console.error('❌ Matrix 3x3 field registration failed:', error);
        }
      } else {
        console.warn('⚠️ Blockly not available for field registration at constructor time.');
      }
    }
  ...
  _createClass(Scratch3SpikePrimeBlocks, [{
      ...
      return {
         ...
         */
        {
          ...
          arguments: {
            ...
            PATTERN: {
              type: 'field_matrix_3x3_color',
              // Uses our custom field
              defaultValue: '960000960960a60960960000960' // Heart pattern
            }
          }
        }, {
          opcode: 'setMatrix3x3Preset',
          text: '📋 [PORT] preset [PRESET]',
          blockType: BlockType.COMMAND,
          arguments: {
            PORT: {
              type: ArgumentType.STRING,
        ...
        menus: {
          ...
          MATRIX_3X3_QUICK: {
            acceptReporters: false,
            items: ['.1', 'r1', 'r2', 'r3', 'r4', 'r5', 'r6', 'r7', 'r8', 'r9', 'r10', 'g1', 'g2', 'g3', 'g4', 'g5', 'g6', 'g7', 'g8', 'g9', 'g10', 'b1', 'b2', 'b3', 'b4', 'b5', 'b6', 'b7', 'b8', 'b9', 'b10', 'y1', 'y2', 'y3', 'y4', 'y5', 'y6', 'y7', 'y8', 'y9', 'y10', 'w1', 'w2', 'w3', 'w4', 'w5', 'w6', 'w7', 'w8', 'w9', 'w10']
          },
          MATRIX_3X3_COLOR: {
            acceptReporters: false,
    ...
    key: "setMovementMotors",
    value: function setMovementMotors(args) {
      this._peripheral._movementMotors = [Cast.toString(args.PORT_A).trim().toUpperCase(), Cast.toString(args.PORT_B).trim().toUpperCase()];
      var _this$_peripheral$_mo = _slicedToArray(this._peripheral._movementMotors, 2),
        portA = _this$_peripheral$_mo[0],
        portB = _this$_peripheral$_mo[1];
      return this._peripheral.sendPythonCommand("from spike import MotorPair; motors = MotorPair('".concat(portA, "', '").concat(portB, "')"));
    }
  }, {
    key: "moveForward",
    value: function moveForward(args) {
      ...
      var _this$_peripheral$_mo2 = _slicedToArray(this._peripheral._movementMotors, 2),
        portA = _this$_peripheral$_mo2[0];
        _this$_peripheral$_mo2[1];
    ...
    value: function steer(args) {
      ...
      var _this$_peripheral$_mo3 = _slicedToArray(this._peripheral._movementMotors, 1),
    ...
    value: function setMovementSpeed(args) {
      ...
      var _this$_peripheral$_mo4 = _slicedToArray(this._peripheral._movementMotors, 2),
        portA = _this$_peripheral$_mo4[0],
        portB = _this$_peripheral$_mo4[1];
    ...
    value: function _motorRunForDegrees(ports, direction, degrees) {
      ...
      var promises = ports.map(function (port) {
        var setting = _this12._peripheral.motorSettings[port];

        // Try both standard command and Python method
        var standardCommand = _this12._peripheral.sendCommand('scratch.motor_run_for_degrees', {
          port: port,
        ...
        var altCommand = _this12._peripheral.sendPythonCommand("import hub; hub.port.".concat(port, ".motor.run_for_degrees(").concat(Math.floor(degrees), ", ").concat(setting.speed * direction, ")"));
    ...
    value: function _motorRunTimed(ports, direction, seconds) {
      var _this13 = this;
      var promises = ports.map(function (port) {
        var setting = _this13._peripheral.motorSettings[port];
        var standardCommand = _this13._peripheral.sendCommand('scratch.motor_run_timed', {
          port: port,
          speed: setting.speed * direction,
        ...
        var altCommand = _this13._peripheral.sendPythonCommand("import hub; hub.port.".concat(port, ".motor.run_for_time(").concat(Math.floor(seconds * 1000), ", ").concat(setting.speed * direction, ")"));
        return standardCommand.catch(function () {
          return altCommand;
        });
      });
    ...
    value: function motorStart(args) {
      ...
      var ports = this._validatePorts(Cast.toString(args.PORT));
      var promises = ports.map(function (port) {
        var setting = _this14._peripheral.motorSettings[port];
        var standardCommand = _this14._peripheral.sendCommand('scratch.motor_start', {
          port: port,
          speed: setting.speed * direction,
          stall: setting.stallDetection
        });
        var altCommand = _this14._peripheral.sendPythonCommand("import hub; hub.port.".concat(port, ".motor.pwm(").concat(Math.round(setting.speed * direction), ")"));
        return standardCommand.catch(function () {
          return altCommand;
        });
      });
    ...
    value: function motorStop(args) {
      ...
      var ports = this._validatePorts(Cast.toString(args.PORT));
      var promises = ports.map(function (port) {
        var setting = _this15._peripheral.motorSettings[port];
        var standardCommand = _this15._peripheral.sendCommand('scratch.motor_stop', {
          port: port,
          stop: setting.stopMode
        });
        var altCommand = _this15._peripheral.sendPythonCommand("import hub; hub.port.".concat(port, ".motor.stop()"));
        return standardCommand.catch(function () {
          return altCommand;
        });
      });
    ...
    value: function motorSetSpeed(args) {
      ...
      ports.forEach(function (port) {
        _this16._peripheral.motorSettings[port].speed = speed;
      });
    }
  }, {
    ...
    /**
     * Set 3x3 LED matrix pattern from interactive pattern designer
     * @param {object} args - Block arguments
     * @param {string} args.PORT - Target port (A-F)
     * @param {string} args.PATTERN - Encoded pattern from matrix field
     * @returns {Promise} Command execution promise
     */
  }, {
    ...
    value: function setMatrix3x3Pattern(args) {
      var port = Cast.toString(args.PORT).trim().toUpperCase();
      var pattern = Cast.toString(args.PATTERN);
      return this._setMatrix3x3FromEncodedPattern(port, pattern);
    }

    /**
     * Set 3x3 matrix to a predefined preset pattern
     * @param {object} args - Block arguments  
     * @param {string} args.PORT - Target port (A-F)
     * @param {string} args.PRESET - Preset pattern name
     * @returns {Promise} Command execution promise
     */
  }, {
    key: "setMatrix3x3Preset",
    value: function setMatrix3x3Preset(args) {
      var port = Cast.toString(args.PORT).trim().toUpperCase();
      var preset = Cast.toString(args.PRESET);
      var patternData = DisplayPatterns[preset] || DisplayPatterns.heart;
      return this._setMatrix3x3FromEncodedPattern(port, patternData);
    }

    /**
     * Internal method to set 3x3 matrix from encoded pattern
     * Decodes the pattern string and sends appropriate commands to hub
     * @param {string} port - Target port 
     * @param {string} encodedPattern - 27-character encoded pattern
     * @returns {Promise} Command execution promise
     */
  }, {
    key: "_setMatrix3x3FromEncodedPattern",
    value: function _setMatrix3x3FromEncodedPattern(port, encodedPattern) {
      try {
        var pixels = [];

        // Decode pattern: every 3 chars = colorId + brightness + reserved  
        for (var i = 0; i < 27; i += 3) {
          var colorId = parseInt(encodedPattern.charAt(i), 16) || 0;
          var brightness = parseInt(encodedPattern.charAt(i + 1), 16) || 5;
          if (colorId === 0) {
            // Off state 
            pixels.push(0x01);
          } else {
            // Combine brightness and color for LEGO protocol
            pixels.push(brightness * 16 + colorId);
          }
        }

        // Ensure exactly 9 pixels
        while (pixels.length < 9) pixels.push(0x01);
        pixels.splice(9);

        // Create byte string for Python command
    ...
    key: "matrix3x3Help",
    value: function matrix3x3Help(args) {
    ...
    /**
     * Clear all LEDs in 3x3 matrix (turn off)
     * @param {object} args - Block arguments
     * @param {string} args.PORT - Target port (A-F)  
     * @returns {Promise} Command execution promise
     */
