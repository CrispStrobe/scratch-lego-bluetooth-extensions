...
(function (module) {
  (function (w) {
    ...
    function findBest(atobNative) {
      ...
      if ('object' === _typeof$2(w.base64js)) {
...
// All enums below are derived from the LEGO Wireless Protocol documentation.
var BoostIO = {
  MOTOR_INTERNAL_TACHO: 0x27,
  MOTOR_EXTERNAL_TACHO: 0x26,
...
var numberToInt32Array = function numberToInt32Array(number) {
  ...
  dataview.setInt32(0, number, true); // true for little-endian
...
var decodeVersion = function decodeVersion(version) {
  // As per section 3.5.6
  var parts = version.toString(16).padStart(8, "0");
  return [parts.substring(0, 2), parts.substring(2, 4), parts.substring(4, 6), parts.substring(6, 8)].join(".");
};
var BoostMotor = /*#__PURE__*/function () {
  function BoostMotor(parent, index) {
    ...
    this._useProfile = 0x00; // Bitmask for accel/decel
    this._status = 'off';
  ...
  _createClass(BoostMotor, [{
    ...
    set: function set(value) {
      this._power = MathUtil.clamp(value, -100, 100);
    }
  }, {
    key: "position",
    get: function get() {
    ...
    value: function setAcceleration(time) {
      ...
      this._useProfile |= 0x01; // Set acceleration bit
      var cmd = this._parent.generateOutputCommand(this._index, 0, BoostOutputSubCommand.SET_ACC_TIME, _toConsumableArray(numberToInt32Array(this._accelerationTime).slice(0, 2)));
    ...
    value: function setDeceleration(time) {
      ...
      this._useProfile |= 0x02; // Set deceleration bit
      var cmd = this._parent.generateOutputCommand(this._index, 0, BoostOutputSubCommand.SET_DEC_TIME, _toConsumableArray(numberToInt32Array(this._decelerationTime).slice(0, 2)));
      this._parent.send(BoostBLE.characteristic, cmd);
    }
  }, {
    key: "resetPosition",
    value: function resetPosition() {
      ...
      // Encoded via WriteDirectModeData as per protocol doc appendix
      var cmd = this._parent.generateOutputCommand(this._index, 0, BoostOutputSubCommand.WRITE_DIRECT_MODE_DATA, [BoostMode.MOTOR_SENSOR].concat(_toConsumableArray(numberToInt32Array(newPosition))));
      return this._parent.send(BoostBLE.characteristic, cmd);
    }
  }, {
    key: "_clearPromise",
    value: function _clearPromise() {
    ...
    key: "turnOn",
    value: function turnOn(power) {
      this._clearPromise();
      this._status = 'on';
      var speed = power || this.power;
      var cmd = this._parent.generateOutputCommand(this._index, BoostOutputExecution.EXECUTE_IMMEDIATELY, BoostOutputSubCommand.START_SPEED, [speed, 100, this._useProfile]);
      this._parent.send(BoostBLE.characteristic, cmd);
    }
  }, {
    key: "turnOff",
    value: function turnOff() {
      ...
      this._clearPromise();
      this._status = 'off';
      var cmd = this._parent.generateOutputCommand(this._index, BoostOutputExecution.EXECUTE_IMMEDIATELY, BoostOutputSubCommand.START_SPEED, [0, 100, this._useProfile]);
      this._parent.send(BoostBLE.characteristic, cmd, useLimiter);
    }
  }, {
    key: "runFor",
    value: function runFor(subCommand, params) {
      var _this = this;
      this._clearPromise();
      this._status = 'on';
      var cmd = this._parent.generateOutputCommand(this._index, BoostOutputExecution.EXECUTE_IMMEDIATELY | BoostOutputExecution.COMMAND_FEEDBACK, subCommand, params);
      return new Promise(function (resolve) {
        _this._pendingPromise = resolve;
        _this._parent.send(BoostBLE.characteristic, cmd);
      });
    }
  }]);
  return BoostMotor;
...
var Boost = /*#__PURE__*/function () {
  ...
  _createClass(Boost, [{
    ...
    value: function reset() {
      ...
      if (this._pingDeviceId) window.clearInterval(this._pingDeviceId);
    }
    ...
    value: function getColor(port) {
      return this._sensors.color[port] || 'none';
    }
  }, {
    key: "getDistance",
    ...
    value: function scan() {
      ...
      }, this._onConnect, this.reset.bind(this));
    }
  }, {
    key: "connect",
    ...
    value: function stopAll() {
      ...
      Object.values(this._motors).forEach(function (motor) {
    ...
    value: function send(uuid, message) {
      ...
      if (!this.isConnected() || useLimiter && !this._rateLimiter.okayToSend()) return Promise.resolve();
    ...
    value: function generateOutputCommand(portID, execution, subCommand, payload) {
      var command = [0x00, BoostMessage.OUTPUT, portID, execution, subCommand].concat(_toConsumableArray(payload));
    ...
    value: function generateInputCommand(portID, mode, delta, enableNotifications) {
      var command = [0x00, BoostMessage.PORT_INPUT_FORMAT_SETUP_SINGLE, portID, mode].concat(_toConsumableArray(numberToInt32Array(delta)), [enableNotifications ? 1 : 0]);
      command.unshift(command.length + 1);
      return command;
    }
  }, {
    ...
    value: function _onConnect() {
      ...
      this._pingDeviceId = window.setInterval(function () {
        return _this2._ble.read(BoostBLE.service, BoostBLE.characteristic, false);
      }, BoostPingInterval);

      // Request all relevant hub properties and enable alerts
      var commands = [[BoostMessage.HUB_PROPERTIES, BoostHubProperty.FW_VERSION, HubPropertyOperation.REQUEST_UPDATE], [BoostMessage.HUB_PROPERTIES, BoostHubProperty.HW_VERSION, HubPropertyOperation.REQUEST_UPDATE], [BoostMessage.HUB_PROPERTIES, BoostHubProperty.BATTERY_VOLTAGE, HubPropertyOperation.ENABLE_UPDATES], [BoostMessage.HUB_PROPERTIES, BoostHubProperty.BUTTON, HubPropertyOperation.ENABLE_UPDATES], [BoostMessage.HUB_PROPERTIES, BoostHubProperty.RSSI, HubPropertyOperation.ENABLE_UPDATES], [BoostMessage.HUB_ALERTS, BoostAlert.LOW_VOLTAGE, BoostAlertOperation.ENABLE_UPDATES], [BoostMessage.HUB_ALERTS, BoostAlert.HIGH_CURRENT, BoostAlertOperation.ENABLE_UPDATES], [BoostMessage.HUB_ALERTS, BoostAlert.OVER_POWER_CONDITION, BoostAlertOperation.ENABLE_UPDATES]];
      commands.forEach(function (c) {
        var cmd = [0].concat(_toConsumableArray(c));
        cmd.unshift(cmd.length + 1);
    ...
    value: function boostColorForIndex(index) {
      return Object.keys(BoostColorIndex).find(function (key) {
    ...
    value: function _onMessage(base64) {
      ...
      switch (messageType) {
        case BoostMessage.HUB_PROPERTIES:
          {
            var property = data[3];
            if (data[4] !== HubPropertyOperation.UPDATE) break;
            switch (property) {
              ...
              case BoostHubProperty.RSSI:
                this._hubStatus.rssi = data.readInt8(5);
                break;
              case BoostHubProperty.FW_VERSION:
                this._hubStatus.fwVersion = decodeVersion(data.readInt32LE(5));
                break;
              case BoostHubProperty.HW_VERSION:
                this._hubStatus.hwVersion = decodeVersion(data.readInt32LE(5));
                break;
            }
            break;
          }
        case BoostMessage.HUB_ALERTS:
          {
            ...
            if (data[4] !== BoostAlertOperation.UPDATE) break;
            var status = data[5] === 0xFF; // 0xFF is Alert, 0x00 is OK
            if (alertType === BoostAlert.LOW_VOLTAGE) this._hubStatus.lowVoltage = status;
            if (alertType === BoostAlert.HIGH_CURRENT || alertType === BoostAlert.OVER_POWER_CONDITION) this._hubStatus.highCurrent = status;
            break;
          }
        case BoostMessage.HUB_ATTACHED_IO:
          {
            var portID = data[3];
            var event = data[4];
            var typeId = event ? data.readUInt16LE(5) : 0;
            if (event === BoostIOEvent.ATTACHED) {
              this._ports[portID] = typeId;
              if (typeId === BoostIO.MOTOR_INTERNAL_TACHO || typeId === BoostIO.MOTOR_EXTERNAL_TACHO) {
                this._motors[portID] = new BoostMotor(this, portID);
                this._setInputMode(portID, BoostMode.MOTOR_SENSOR);
              } else if (typeId === BoostIO.COLOR_DISTANCE_SENSOR) this._setInputMode(portID, BoostMode.COLOR);else if (typeId === BoostIO.TECHNIC_FORCE_SENSOR) this._setInputMode(portID, BoostMode.FORCE);else if (typeId === BoostIO.TILT_INTERNAL) this._setInputMode(portID, BoostMode.TILT);
        ...
        case BoostMessage.PORT_VALUE:
          {
            var _portID = data[3];
            var type = this._ports[_portID];
            var mode = this._portModes[_portID];
            if (type === BoostIO.TILT_INTERNAL) {
              this._sensors.tiltX = data.readInt8(4);
              this._sensors.tiltY = data.readInt8(5);
            } else if (type === BoostIO.COLOR_DISTANCE_SENSOR) {
              if (mode === BoostMode.COLOR) this._sensors.color[_portID] = this.boostColorForIndex(data[4]);else if (mode === BoostMode.DISTANCE) this._sensors.distance[_portID] = data[4] * 10;else if (mode === BoostMode.REFLECTION) this._sensors.reflection[_portID] = data[4];
            } else if (type === BoostIO.TECHNIC_FORCE_SENSOR) {
              if (mode === BoostMode.FORCE) this._sensors.force[_portID] = data[4] / 10;
              if (mode === BoostMode.TOUCHED) this._sensors.pressed[_portID] = data[4] > 0;
            } else if (type === BoostIO.MOTOR_INTERNAL_TACHO || type === BoostIO.MOTOR_EXTERNAL_TACHO) {
              if (this.motor(_portID)) this.motor(_portID).position = int32ArrayToNumber(data.slice(4, 8));
            }
            break;
          }
        case BoostMessage.PORT_FEEDBACK:
          {
            var _portID2 = data[3];
            if (this.motor(_portID2) && data[4] & BoostPortFeedback.COMPLETED) this.motor(_portID2)._clearPromise();
            break;
          }
      }
    }
...
var Scratch3BoostBlocks = /*#__PURE__*/function () {
  ...
  _createClass(Scratch3BoostBlocks, [{
    ...
    value: function getInfo() {
      return {
        ...
        blocks: [{
          ...
          text: 'run motor [MOTOR_ID] for [DURATION] seconds',
          arguments: {
            MOTOR_ID: {
          ...
          blockType: BlockType.COMMAND,
          text: 'run motor [MOTOR_ID] for [ROTATION] rotations',
          arguments: {
            MOTOR_ID: {
              type: ArgumentType.STRING,
              menu: 'MOTOR_ID',
          ...
          blockType: BlockType.COMMAND,
          text: 'motor [MOTOR_ID] go to position [POSITION]',
          arguments: {
            MOTOR_ID: {
              type: ArgumentType.STRING,
              menu: 'MOTOR_ID',
          ...
          opcode: 'motorOn',
          blockType: BlockType.COMMAND,
          text: 'run motor [MOTOR_ID] with power [POWER]%',
          arguments: {
            MOTOR_ID: {
              type: ArgumentType.STRING,
              menu: 'MOTOR_ID_ALL',
          ...
          opcode: 'motorOnPairFor',
          blockType: BlockType.COMMAND,
          text: 'run motors [MOTOR_A] & [MOTOR_B] for [DEGREES] degrees with power [POWER_A] & [POWER_B]',
          arguments: {
            MOTOR_A: {
              type: ArgumentType.STRING,
              menu: 'MOTOR_ID',
          ...
          blockType: BlockType.HAT,
          text: 'when [PORT] force sensor is pressed',
          arguments: {
            PORT: {
              type: ArgumentType.STRING,
              menu: 'SENSOR_PORTS',
          ...
          opcode: 'isTilted',
          blockType: BlockType.BOOLEAN,
          text: 'is tilted [TILT_DIRECTION_ANY]?',
          arguments: {
            TILT_DIRECTION_ANY: {
              type: ArgumentType.STRING,
              menu: 'TILT_DIRECTION_ANY',
          ...
          blockType: BlockType.HAT,
          text: 'when a motor is overloaded'
        }],
        menus: {
          MOTOR_ID: this._makeMotorMenu(['A', 'B', 'C', 'D']),
          MOTOR_ID_ALL: this._makeMotorMenu(['A', 'B', 'C', 'D', 'A+B', 'ALL']),
          SENSOR_PORTS: this._makeMotorMenu(['A', 'B', 'C', 'D']),
          STOP_ACTION: ['float', 'brake', 'hold'],
          TILT_DIRECTION: ['up', 'down', 'left', 'right'],
          TILT_DIRECTION_ANY: ['up', 'down', 'left', 'right', 'any'],
          COLOR: ['any', 'red', 'blue', 'green', 'yellow', 'white', 'black']
        }
      };
    }
  }, {
    key: "_makeMotorMenu",
    value: function _makeMotorMenu(items) {
      return {
    ...
    key: "_getMotorPorts",
    value: function _getMotorPorts(motorID) {
      switch (motorID) {
        case 'A':
          return [BoostPort.A];
        case 'B':
        ...
        case 'D':
          return [BoostPort.D];
        case 'A+B':
          return [BoostPort.A, BoostPort.B];
        case 'ALL':
          return Object.values(this._peripheral._motors).map(function (m) {
            return m._index;
          });
        default:
          return [];
    ...
    value: function _forEachMotor(motorID, callback) {
      var _this3 = this;
      var ports = this._getMotorPorts(motorID);
      ports.forEach(function (port) {
        if (_this3._peripheral.motor(port)) callback(_this3._peripheral.motor(port));
      });
    }
    ...
    value: function motorOnFor(args) {
      ...
      var promises = [];
      this._forEachMotor(args.MOTOR_ID, function (motor) {
        promises.push(motor.runFor(BoostOutputSubCommand.START_SPEED_FOR_TIME, [].concat(_toConsumableArray(numberToInt32Array(durationMS).slice(0, 2)), [motor.power, 100, motor.stopMode])));
      });
    ...
    value: function motorOnForRotation(args) {
      ...
      var promises = [];
      this._forEachMotor(args.MOTOR_ID, function (motor) {
        promises.push(motor.runFor(BoostOutputSubCommand.START_SPEED_FOR_DEGREES, [].concat(_toConsumableArray(numberToInt32Array(degrees)), [motor.power, 100, motor.stopMode, motor._useProfile])));
      });
      return Promise.all(promises);
    }
  }, {
    ...
    value: function motorRunToPosition(args) {
      ...
      var promises = [];
      this._forEachMotor(args.MOTOR_ID, function (motor) {
        promises.push(motor.runFor(BoostOutputSubCommand.GO_TO_ABS_POSITION, [].concat(_toConsumableArray(numberToInt32Array(position)), [motor.power, 100, motor.stopMode, motor._useProfile])));
      });
      return Promise.all(promises);
    }
  }, {
    ...
    value: function motorOn(args) {
      ...
      this._forEachMotor(args.MOTOR_ID, function (motor) {
        return motor.turnOn(power);
      });
      return new Promise(function (resolve) {
        return setTimeout(resolve, BoostBLE.sendInterval);
      });
    }
  }, {
    key: "motorOff",
    ...
    value: function setMotorPower(args) {
      ...
      this._forEachMotor(args.MOTOR_ID, function (motor) {
        motor.power = power;
        if (motor._status === 'on') motor.turnOn();
      });
    }
  }, {
    ...
    value: function setMotorAcceleration(args) {
      this._forEachMotor(args.MOTOR_ID, function (motor) {
        return motor.setAcceleration(Cast.toNumber(args.TIME));
      });
    }
  }, {
    ...
    value: function setMotorDeceleration(args) {
      this._forEachMotor(args.MOTOR_ID, function (motor) {
        return motor.setDeceleration(Cast.toNumber(args.TIME));
      });
    }
  }, {
    ...
    value: function resetMotorPosition(args) {
      this._forEachMotor(args.MOTOR_ID, function (motor) {
        return motor.resetPosition(Cast.toNumber(args.POSITION));
      });
    }
  }, {
    key: "getMotorPosition",
    value: function getMotorPosition(args) {
      var port = this._getMotorPorts(args.MOTOR_ID)[0];
      var motor = this._peripheral.motor(port);
      return motor ? motor.position : 0;
    }
  }, {
    key: "motorOnPairFor",
    value: function motorOnPairFor(args) {
      var portA = this._getMotorPorts(args.MOTOR_A)[0];
      var portB = this._getMotorPorts(args.MOTOR_B)[0];
      var motorA = this._peripheral.motor(portA);
      var motorB = this._peripheral.motor(portB);
      if (!motorA || !motorB) return Promise.resolve();
      var cmd = this._peripheral.generateOutputCommand(portA | portB << 8, BoostOutputExecution.EXECUTE_IMMEDIATELY | BoostOutputExecution.COMMAND_FEEDBACK, BoostOutputSubCommand.START_SPEED_FOR_DEGREES_PAIR, [].concat(_toConsumableArray(numberToInt32Array(Cast.toNumber(args.DEGREES))), [Cast.toNumber(args.POWER_A), Cast.toNumber(args.POWER_B), 100, motorA.stopMode, motorA._useProfile]));
      var promiseA = new Promise(function (res) {
        return motorA.pendingPromise = res;
      });
      var promiseB = new Promise(function (res) {
        return motorB.pendingPromise = res;
      });
      ...
      var _checkColor2 = _asyncToGenerator( /*#__PURE__*/regenerator.mark(function _callee2(args) {
        ...
        return regenerator.wrap(function _callee2$(_context2) {
          while (1) switch (_context2.prev = _context2.next) {
            case 0:
              port = this._getMotorPorts(args.PORT)[0];
            ...
            case 5:
              ...
              return _context2.abrupt("return", args.COLOR === 'any' ? currentColor !== 'none' : currentColor === args.COLOR);
            case 7:
            case "end":
    ...
    value: function _isTilted(direction) {
      var _this$_peripheral = this._peripheral,
        tiltX = _this$_peripheral.tiltX,
        tiltY = _this$_peripheral.tiltY;
      var threshold = Scratch3BoostBlocks.TILT_THRESHOLD;
      switch (direction) {
        case 'up':
          return tiltY < -threshold;
        case 'down':
          return tiltY > threshold;
        case 'left':
        ...
        case 'right':
          return tiltX < -threshold;
        // Tilt values are inverted for left/right
        case 'any':
    ...
    value: function getTiltAngle(args) {
      var _this$_peripheral2 = this._peripheral,
        tiltX = _this$_peripheral2.tiltX,
        tiltY = _this$_peripheral2.tiltY;
      switch (args.TILT_DIRECTION) {
        case 'up':
          return -tiltY;
        case 'down':
          return tiltY;
    ...
    value: function setLightHue(args) {
      var hue = Cast.toNumber(args.HUE) * 360 / 100;
      var rgb = color.hsvToRgb({
        h: hue,
        s: 1,
        v: 1
      });
      var cmd = this._peripheral.generateOutputCommand(BoostPort.LED, 0, BoostOutputSubCommand.WRITE_DIRECT_MODE_DATA, [1, rgb.r, rgb.g, rgb.b]);
      return this._peripheral.send(BoostBLE.characteristic, cmd);
    }
  }, {
    key: "shutdown",
    value: function shutdown() {
    ...
    value: function whenMotorOverloaded() {
      return this._peripheral.hubStatus.highCurrent;
    }
  }], [{
    key: "EXTENSION_ID",
    get: function get() {
      return 'boostUltimate';
    }
  }, {
    key: "TILT_THRESHOLD",
    get: function get() {
